...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 3AC4B69AED
	for <lists+linux-kernel@lfdr.de>; Mon, 15 Jul 2019 20:38:25 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729671AbfGOSiR (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 15 Jul 2019 14:38:17 -0400
Received: from mail-lf1-f42.google.com ([209.85.167.42]:38206 "EHLO
        mail-lf1-f42.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1729277AbfGOSiQ (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 15 Jul 2019 14:38:16 -0400
Received: by mail-lf1-f42.google.com with SMTP id h28so11719387lfj.5
        for <linux-kernel@vger.kernel.org>; Mon, 15 Jul 2019 11:37:55 -0700 (PDT)
...
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        h=x-gm-message-state:mime-version:from:date:message-id:subject:to:cc
         :content-transfer-encoding;
        bh=abpGYdCpJvHz1vjFGt7226BiwUwVd0W4QaOFERQOYh8=;
        b=
X-Gm-Message-State: APjAAAVpuPQR6sc01Rw2R1g251w/2HyhjKnwbzUfJ2/gwz2g+5kKl8VW
        wOgsP7OdeU4sLaTv6qMJk/hie8AvWQJAWEUljxQ=
X-Google-Smtp-Source: APXvYqwFxCxRmI7T0Clz+7kvlSiz/fRvvcWMF7c6pthJGPRIMThz0B2U1lwrkvN16vWomSWPh0rlYZJrk9NcGqyhYQ4=
X-Received: by 2002:a19:ccc6:: with SMTP id c189mr12078495lfg.160.1563215872503;
...
This is the updated pull request with all the vmware and mm changes
removed from it with a revert. I'd rebuild the tree but we'd be
reducing the testing confidence quite seriously if we did, where as
revert lets the shame be apparent for all for ever. I'll talk to
Thomas about making sure he gets better feedback on core changes and
I'll talk to myself about making sure I go with my bad feelings on
things earlier rather than leave it too late.
...
I've created a branch along with all the other conflicts.

https://cgit.freedesktop.org/drm/drm/log/?h=3Ddrm-next-5.3-backmerge-confli=
cts
...
are available in the Git repository at:
  ...
  git://anongit.freedesktop.org/drm/drm tags/drm-next-2019-07-16

for you to fetch changes up to 3729fe2bc2a01f4cc1aa88be8f64af06084c87d6:
...
Alex Deucher (37):
      ...
      drm/amd/display: add fast_validate parameter to dcn20_validate_bandwi=
dth
...
Chris Wilson (150):
      ...
      drm/i915: Disable preemption and sleeping while using the punit sideb=
and
      ...
      drm/i915: Enable render context support for gen4 (Broadwater to Canti=
ga)
      drm/i915/gvt: Pin the per-engine GVT shadow contexts
      drm/i915: Export intel_context_instance()
      drm/i915/selftests: Use the real kernel context for sseu isolation te=
sts
      ...
      drm/i915: Complete both freed-object passes before draining the workq=
ueue
      ...
      drm/i915: Assert breadcrumbs are correctly ordered in the signal hand=
ler
      ...
      drm/i915: Only reschedule the submission tasklet if preemption is pos=
sible
...
[]
      ...
      drm/i915/gtt: Neuter the deferred unbind callback from gen6_ppgtt_cle=
anup
      ...
      drm/i915: Move GEM object domain management from struct_mutex to loca=
l
      ...
      drm/i915: Make default value for i915.mmio_debug a compile time optio=
n
      ...
      dma-buf: Discard old fence_excl on retrying get_fences_rcu for reallo=
c
...
Christian K=C3=B6nig (18):
      drm/i915: remove DRM_AUTH from IOCTLs which also have DRM_RENDER_ALLO=
W
...
Dan Carpenter (5):
      ...
      drm/bridge: sii902x: re-order conditions to prevent out of bounds rea=
d
...
Daniele Ceraolo Spurio (12):
      ...
      drm/i915: make enable/disable rpm assert function use the rpm structu=
re
...
Derek Lai (1):
      drm/amd/display: add i2c_hw_Status check to make sure as HW I2c in us=
e
...
Dongli Zhang (1):
      drm/i915: remove unused IO_TLB_SEGPAGES which should be defined by sw=
iotlb
...
Douglas Anderson (7):
      ...
      drm/bridge/synopsys: dw-hdmi: Fix unwedge crash when no pinctrl entri=
es
...
Eric Anholt (11):
      ...
      drm/v3d: Drop reservation of a shared slot in the dma-buf reservation=
s.
      drm/v3d: Add missing implicit synchronization.
      drm/doc: Allow new UAPI to be used once it's in drm-next/drm-misc-nex=
t.
...
Evan Quan (33):
      ...
      drm/amd/powerplay: support ppfeatures sysfs interface on sw smu routi=
ne
      ...
      drm/amd/powerplay: support sw smu hotspot and memory temperature retr=
ieval
      drm/amd/powerplay: fix sw SMU wrong UVD/VCE powergate setting
      drm/amd/powerplay: enable ppfeaturemask module parameter support on V=
ega20
...
Fernando Pacheco (5):
      ...
      drm/i915/selftests: Check that gpu reset is usable from atomic contex=
t
...
Greg Kroah-Hartman (17):
      vga_switcheroo: no need to check return value of debugfs_create funct=
ions
      panel: rocktech: no need to check return value of debugfs_create func=
tions
      ...
      msm: adreno: no need to check return value of debugfs_create function=
s
...
Hans de Goede (7):
      drm/i915/dsi: Call drm_connector_cleanup on vlv_dsi_init error exit p=
ath
...
Hariprasad Kelam (2):
      drm/bridge: analogix_dp: possible condition with no effect (if =3D=3D=
 else)
...
Hawking Zhang (83):
      ...
      drm/amdgpu: add gfx10 specific new member pa_sc_tile_steering_overrid=
e
      ...
      drm/amdgpu/psp: start rlc autoload after psp recieved all gfx firmwar=
e
      drm/amdgpu/psp: switch to use sos_offset_bytes member as sys_bin_size
      drm/amdgpu/psp: perform tmr_init and asd_init after loading sysdrv/so=
s
      ...
      drm/amdgpu: add pa_sc_tile_steering_override to drm_amdgpu_info_devic=
e
...
functions setting
      ...
      drm/amd/powerplay: smu needs to be initialized after rlc in direct mo=
de
...
pptable for navi10 (v2)
      ...
      drm/amd/powerplay: introduce smu clk type to handle ppclk for each as=
ic
...
Jani Nikula (69):
      ...
      drm/i915: extract intel_gmbus.h from i915_drv.h and rename intel_i2c.=
c
      ...
      drm/i915/bios: use port info child pointer to determine LSPCON presen=
ce
...
Jayant Shekhar (3):
      ...
      dt-bindings: msm/disp: Introduce interconnect bindings for MDSS on SD=
M845
...
Jordan Crouse (7):
      drm/msm/adreno: Enable 64 bit mode by default on a5xx and a6xx target=
s
...
Joshua Aberback (8):
      ...
      drm/amd/display: Program VTG params after programming Global Sync for=
 DCN2
...
Kenneth Feng (15):
      ...
      drm/amd/powerplay: enable gfxclk ds,dcefclk ds and fw dstate on navi1=
0
...
Kevin Wang (62):
      drm/amd/powerplay: add helper function to get smu firmware & if versi=
on
...
for navi10
      ...
      drm/amd/powerplay: add function get_clock_by_type_with_latency for na=
vi10
      drm/amd/powerplay: add function pre_display_config_changed for navi10
      drm/amd/powerplay: add function display_configuration_changed for nav=
i10
      ...
      drm/amd/powerplay: add function get_current_activity_percent for navi=
10
      drm/amd/powerplay: move read sensor of UVD[VCE]_POWER to amdgpu_smu f=
ile
...
for navi10
      drm/amd/powerplay: add function set_watermarks_table function for nav=
i10
      drm/amd/powerplay: add function read_sensor for navi10
      drm/amd/powerplay: fix dpm freq unit error (10KHz -> Mhz)
      drm/amd/powerplay: simplify the interface of get_current_activity_per=
cent
...
amdgpu_smu file
      ...
      drm/amd/powerplay: move function thermal_get_temperature to veag20_pp=
t
      ...
      drm/amd/powerplay: use pp_feature_mask to control uclk(mclk) dpm enab=
led
...
Laurent Pinchart (11):
      ...
      dt-bindings: display: bridge: thc63lvd1024: Document dual-link operat=
ion
      ...
      drm: rcar-du: Skip LVDS1 output on Gen3 when using dual-link LVDS mod=
e
...
Le.Ma (3):
      drm/amdgpu: add structures for buffer allocate/release for rlc autolo=
ad
...
Lowry Li (Arm Technology China) (10):
      ...
      drm/komeda: Adds limitation check for AFBC wide block not support Rot=
90
...
Lucas De Marchi (16):
      ...
      drm/i915/dmc: extract fw_info and table walk from intel_package_heade=
r
...
.
      drm/rockchip: Convert to using __drm_atomic_helper_crtc_reset() for r=
eset.
      drm/tegra: Convert to using __drm_atomic_helper_crtc_reset() for rese=
t.
...
Marek Ol=C5=A1=C3=A1k (5):
      ...
      drm/amdgpu: don't invalidate caches in RELEASE_MEM, only do the write=
back
...
Maxime Ripard (21):
      ...
      drm/fourcc: Pass the format_info pointer to drm_format_plane_width/he=
ight
      ...
      drm/modes: Allow to specify rotation and reflection on the commandlin=
e
...
dsi_phy_hw_v3_0_lane_settings
      ...
      drm/amd/powerplay: Zero initialize freq in smu_v11_0_get_current_clk_=
freq
...
Nicholas Kazlauskas (23):
      ...
      drm/amd/display: Don't set mode_changed=3Dfalse if the stream was rem=
oved
      ...
      drm/amd/display: Use current connector state if NULL when checking bp=
c
...
Oak Zeng (43):
      ...
      drm/amdkfd: Initialize HSA_CAP_ATS_PRESENT capability in topology cod=
es
...
Sandor Yu (1):
      drm/rockchip: cdn-dp: correct rate in the struct drm_dp_link assignme=
nt
...
drm_atomic_state
      ...
      drm/connector: Fix kerneldoc warning in HDR_OUTPUT_METADATA descripti=
on
...
Thomas Hellstrom (11):
      ...
      drm/ttm: Allow the driver to provide the ttm struct vm_operations_str=
uct
...
Thomas Zimmermann (36):
      ...
      drm: Add |struct drm_gem_vram_object| callbacks for |struct ttm_bo_dr=
iver|
      drm: Add |struct drm_gem_vram_object| callbacks for |struct drm_drive=
r|
      ...
      drm/vboxvideo: Convert vboxvideo driver to |struct drm_gem_vram_objec=
t|
      drm/vboxvideo: Convert vboxvideo driver to VRAM MM
      drm/hisilicon: Convert hibmc-drm driver to |struct drm_gem_vram_objec=
t|
...
Tomi Valkeinen (27):
      ...
      drm/bridge: tc358767: move PXL PLL enable/disable to stream enable/di=
sable
      ...
      drm/bridge: tc358767: copy the mode data, instead of storing the poin=
ter
...
Tony Cheng (1):
      drm/amd/display: move dsc clock from plane_resource to stream_resourc=
e
...
Tvrtko Ursulin (27):
      ...
      drm/i915/guc: Move intel_guc_reserved_gtt_size to intel_wopcm_guc_siz=
e
...
Ville Syrj=C3=A4l=C3=A4 (70):
      ...
      drm/i915: Bump fb stride limit to 128KiB for gen4+ and 256KiB for gen=
7+
...
Xiaojie Yuan (15):
      ...
      drm/amdgpu/discovery: update definitions of table_info and binary_hea=
der
...
async_gfx_ring is disabled
      drm/amd/display: use fixed-width data type for soc bounding box struc=
t
...
Yrjan Skrimstad (1):
      drm/amd/powerplay/smu7_hwmgr: replace blocking delay with non-blockin=
g
...
is enumulated
      drm/amdgpu: Use heavy weight for tlb invalidation on xgmi configurati=
on
...
++++
 ...
 drivers/gpu/drm/ttm/ttm_bo.c                       |    270 +-
 ...
 1760 files changed, 477684 insertions(+), 35662 deletions(-)
 create mode 100644
Documentation/devicetree/bindings/display/allwinner,sun6i-a31-mipi-dsi.yaml
 create mode 100644 Documentation/devicetree/bindings/display/ingenic,lcd.t=
xt
 create mode 100644
...
93%)
 ...
 rename drivers/gpu/drm/{i915/i915_gemfs.h =3D> amd/amdgpu/nbio_v2_3.h} (52=
%)
 ...
 create mode 100644 drivers/gpu/drm/amd/amdkfd/kfd_device_queue_manager_v10=
.c
 ...
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dce100/dce_clk_m=
gr.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dce100/dce_clk_m=
gr.h
 create mode 100644
...
clk_mgr/dcn10/rv1_clk_mgr.c} (59%)
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn10/rv1_clk_mg=
r.h
 create mode 100644
...
drivers/gpu/drm/amd/display/dc/clk_mgr/dcn10/rv1_clk_mgr_vbios_smu.h
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn10/rv2_clk_mg=
r.c
 rename drivers/gpu/drm/amd/display/dc/{dcn10/dcn10_clk_mgr.h =3D>
clk_mgr/dcn10/rv2_clk_mgr.h} (82%)
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn20/dcn20_clk_=
mgr.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn20/dcn20_clk_=
mgr.h
 ...
 create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_link_encoder=
.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_link_encoder=
.h
 ...
 create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_stream_encod=
er.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_stream_encod=
er.h
...
drivers/gpu/drm/amd/display/dc/dml/dcn20/display_rq_dlg_calc_20.h
 ...
 create mode 100644 drivers/gpu/drm/amd/display/dc/gpio/dcn20/hw_factory_dc=
n20.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/gpio/dcn20/hw_factory_dc=
n20.h
 create mode 100644
...
inc/hw/clk_mgr_internal.h} (51%)
 ...
 create mode 100644 drivers/gpu/drm/amd/display/dc/irq/dcn20/irq_service_dc=
n20.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/irq/dcn20/irq_service_dc=
n20.h
...
drivers/gpu/drm/amd/include/asic_reg/athub/athub_2_0_0_sh_mask.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/clk/clk_11_0_0_off=
set.h
 create mode 100644
drivers/gpu/drm/amd/include/asic_reg/clk/clk_11_0_0_sh_mask.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/dcn/dcn_2_0_0_offs=
et.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/dcn/dcn_2_0_0_sh_m=
ask.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_1_0_defau=
lt.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_1_0_offse=
t.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_1_0_sh_ma=
sk.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/hdp/hdp_5_0_0_offs=
et.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/hdp/hdp_5_0_0_sh_m=
ask.h
 create mode 100644
...
drivers/gpu/drm/amd/include/asic_reg/mmhub/mmhub_2_0_0_sh_mask.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/nbio/nbio_2_3_defa=
ult.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/nbio/nbio_2_3_offs=
et.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/nbio/nbio_2_3_sh_m=
ask.h
 create mode 100644
...
drivers/gpu/drm/amd/include/asic_reg/smuio/smuio_11_0_0_sh_mask.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/vcn/vcn_2_0_0_offs=
et.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/vcn/vcn_2_0_0_sh_m=
ask.h
...
100%)
 create mode 100644 drivers/gpu/drm/amd/include/ivsrcid/gfx/irqsrcs_gfx_10_=
1.h
 create mode 100644
...
drivers/gpu/drm/amd/include/ivsrcid/sdma1/irqsrcs_sdma1_5_0.h
 create mode 100644 drivers/gpu/drm/amd/include/ivsrcid/vcn/irqsrcs_vcn_2_0=
.h
 ...
 create mode 100644 drivers/gpu/drm/amd/powerplay/inc/smu11_driver_if_navi1=
0.h
 ...
 create mode 100644 drivers/gpu/drm/arm/display/komeda/komeda_wb_connector.=
c
 ...
 rename drivers/gpu/drm/i915/{ =3D> display}/intel_dsi_dcs_backlight.c (99%=
)
 ...
 create mode 100644 drivers/gpu/drm/i915/gem/selftests/i915_gem_client_blt.=
c
 rename drivers/gpu/drm/i915/{ =3D> gem}/selftests/i915_gem_coherency.c (83=
%)
...
gem/selftests/i915_gem_mman.c} (67%)
 ...
 create mode 100644 drivers/gpu/drm/i915/gem/selftests/i915_gem_object_blt.=
c
 ...
 rename drivers/gpu/drm/i915/{intel_ringbuffer.h =3D> gt/intel_engine.h} (9=
0%)
...
gt/selftest_hangcheck.c} (88%)
 rename drivers/gpu/drm/i915/{selftests/intel_lrc.c =3D> gt/selftest_lrc.c}=
 (68%)
