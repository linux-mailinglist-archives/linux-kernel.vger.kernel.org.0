...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 3C584F7973
	for <lists+linux-kernel@lfdr.de>; Mon, 11 Nov 2019 18:05:56 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726962AbfKKRFw (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 11 Nov 2019 12:05:52 -0500
Received: from mail-eopbgr770042.outbound.protection.outlook.com ([40.107.77.42]:65325
        "EHLO NAM02-SN1-obe.outbound.protection.outlook.com"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1726845AbfKKRFw (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 11 Nov 2019 12:05:52 -0500
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 11 Nov 2019 17:04:49.9195
 (UTC)
...
not click links or open attachments unless you recognize the sender and=20
...
> v2 fixes the error and includes all the changes related to this patch=20
in
> the driver.
>=20
...
> @@ -239,7 +239,7 @@ static int bh_work_tx(struct wfx_dev *wdev, int=20
max_msg)
>   */
...
> @@ -107,7 +107,7 @@ static void wfx_tx_policy_build(struct wfx_vif=20
...
>                 if (rates[i].idx < 0)
>                         break;
> @@ -477,7 +477,7 @@ static void wfx_tx_manage_pm(struct wfx_vif *wvif,=20
...
>                                       struct ieee80211_hdr *hdr)
>  {
> @@ -542,11 +542,11 @@ static void wfx_tx_fixup_rates(struct=20
...
>                                     tx_info->driver_rates,=20
&tx_policy_renew);
> @@ -584,7 +584,7 @@ static struct hif_ht_tx_parameters=20
...
>  {
>         // FIXME: ieee80211_get_tid(hdr) should be sufficient for all=20
cases.
...
>  #define HIF_API_MAX_TEMPLATE_FRAME_SIZE                             =20
700
>=20
...
> @@ -452,7 +452,8 @@ int hif_sl_config(struct wfx_dev *wdev, const=20
unsigned long *bitmap)
>         return ret;
>  }
>=20
> -int hif_sl_set_mac_key(struct wfx_dev *wdev, const uint8_t *slk_key,=20
int destination)
...
>=20
>  struct wfx_hif_cmd {
> @@ -59,9 +59,10 @@ int hif_beacon_transmit(struct wfx_vif *wvif, bool=20
enable);
>  int hif_map_link(struct wfx_vif *wvif, u8 *mac_addr, int flags, int=20
sta_id);
>  int hif_update_ie(struct wfx_vif *wvif, const struct hif_ie_flags=20
*target_frame,
...
> -                        const uint8_t *pubkey, const uint8_t=20
*pubkey_hmac);
...
> @@ -145,7 +145,7 @@ static inline int hif_set_mfp(struct wfx_vif=20
...
> @@ -31,7 +31,7 @@ static void wfx_free_key(struct wfx_dev *wdev, int=20
idx)
...
>                              struct ieee80211_key_conf *key, u8=20
*peer_addr)
>  {
>         WARN(key->keylen > sizeof(msg->key_data), "inconsistent=20
data");
> @@ -41,7 +41,7 @@ static uint8_t fill_wep_pair(struct=20
...
>         WARN(key->keylen > sizeof(msg->key_data), "inconsistent=20
data");
> @@ -51,10 +51,10 @@ static uint8_t fill_wep_group(struct=20
...
> @@ -68,18 +68,20 @@ static uint8_t fill_tkip_pair(struct=20
...
> @@ -91,7 +93,7 @@ static uint8_t fill_tkip_group(struct=20
...
*peer_addr)
>  {
>         WARN(key->keylen !=3D sizeof(msg->aes_key_data), "inconsistent=20
data");
> @@ -100,7 +102,7 @@ static uint8_t fill_ccmp_pair(struct=20
...
>                                struct ieee80211_key_seq *seq)
>  {
> @@ -112,10 +114,10 @@ static uint8_t fill_ccmp_group(struct=20
...
>                             + sizeof(msg->mic_key_data), "inconsistent=20
data");
> @@ -127,10 +129,10 @@ static uint8_t fill_sms4_pair(struct=20
...
>                             + sizeof(msg->mic_key_data), "inconsistent=20
data");
> @@ -141,7 +143,7 @@ static uint8_t fill_sms4_group(struct=20
...
> @@ -25,12 +25,14 @@ static inline int wfx_sl_decode(struct wfx_dev=20
*wdev, struct hif_sl_msg *m)
>         return -EIO;
>  }
>=20
> -static inline int wfx_sl_encode(struct wfx_dev *wdev, struct hif_msg=20
...
>  }
>=20
> -static inline int wfx_sl_check_pubkey(struct wfx_dev *wdev, uint8_t=20
*ncp_pubkey, uint8_t *ncp_pubmac)
> +static inline int wfx_sl_check_pubkey(struct wfx_dev *wdev, u8=20
*ncp_pubkey,
> +                                     u8 *ncp_pubmac)
>  {
>         return -EIO;
...
> @@ -456,7 +456,7 @@ void wfx_flush(struct ieee80211_hw *hw, struct=20
ieee80211_vif *vif,
>=20
>  /* WSM callbacks */
>=20
> -static void wfx_event_report_rssi(struct wfx_vif *wvif, uint8_t=20
raw_rcpi_rssi)
> +static void wfx_event_report_rssi(struct wfx_vif *wvif, u8=20
raw_rcpi_rssi)
>  {
...
>=20
>  static void wfx_set_mfp(struct wfx_vif *wvif, struct cfg80211_bss=20
*bss)
>  {
> -       const int pairwise_cipher_suite_count_offset =3D 8 /=20
sizeof(uint16_t);
...
>         bool mfpr =3D false;
>=20
> @@ -606,7 +606,8 @@ static void wfx_set_mfp(struct wfx_vif *wvif,=20
struct cfg80211_bss *bss)
>=20
>         rcu_read_lock();
>         if (bss)
> -               ptr =3D (const uint16_t *) ieee80211_bss_get_ie(bss,=20
WLAN_EID_RSN);
...
> @@ -179,11 +179,11 @@ static inline int wvif_count(struct wfx_dev=20
*wdev)
>         return ret;
>  }
>=20
...
>         while (lo < hi) {
>                 swap =3D *lo;
> @@ -194,7 +194,7 @@ static inline void memreverse(uint8_t *src,=20
uint8_t length)
>=20
...
I know that uXX is prefered over uintXX_t. However, I dislike to change=20
650 lines of code only for this purpose in one batch. It will generate=20
plenty of conflicts with branches currently in development.=20
