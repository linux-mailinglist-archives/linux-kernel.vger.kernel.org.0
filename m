...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id BCAB1F42EC
	for <lists+linux-kernel@lfdr.de>; Fri,  8 Nov 2019 10:16:13 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1730692AbfKHJQL (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Fri, 8 Nov 2019 04:16:11 -0500
Received: from mail-eopbgr760115.outbound.protection.outlook.com ([40.107.76.115]:28233
        "EHLO NAM02-CY1-obe.outbound.protection.outlook.com"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1730281AbfKHJQL (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Fri, 8 Nov 2019 04:16:11 -0500
...
x-forefront-antispam-report: SFV:NSPM;SFS:(10019020)(7916004)(396003)(366004)(376002)(136003)(39840400004)(346002)(189003)(199004)(33716001)(1076003)(71200400001)(256004)(486006)(8676002)(5024004)(54906003)(2906002)(6486002)(107886003)(6512007)(14444005)(11346002)(446003)(33656002)(6436002)(229853002)(4326008)(476003)(25786009)(102836004)(66066001)(9686003)(6246003)(8936002)(26005)(7416002)(30864003)(52116002)(7736002)(66446008)(53546011)(66476007)(66556008)(66946007)(99286004)(316002)(5660300002)(86362001)(71190400001)(186003)(305945005)(386003)(81166006)(561924002)(6916009)(81156014)(3846002)(6506007)(76176011)(14454004)(6116002)(64756008)(478600001)(569006);DIR:OUT;SFP:1102;SCL:1;SRVR:SN6PR04MB4942;H:SN6PR04MB4543.namprd04.prod.outlook.com;FPR:;SPF:None;LANG:en;PTR:InfoNoRecords;MX:1;A:1;
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 08 Nov 2019 08:56:54.1751
 (UTC)
...
designed
> > +               for portable devices. It converts MIPI/DPI to DisplayPo=
rt1.3 4K.
...
> > +       int ret;
> > +
> > +       if (client =3D=3D ctx->last_client)
> > +               return 0;
> > +
...
> > +       i2c_access_workaround(ctx, client);
> > +
> > +       ret =3D i2c_smbus_read_i2c_block_data(client, reg_addr, len, bu=
f);
> > +       if (ret < 0)
...
> > +{
> > +       return anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, AP_AUX_CTRL=
_STATUS);
> > +}
> > +
...
> > +                                AUDIO_CONTROL_REGISTER, ~TDM_TIMING_MO=
DE);
> > +       /* audio change flag */
> > +       ret |=3D anx7625_write_or(ctx, ctx->i2c.rx_p0_client,
...
> > +               *post_divider =3D 12;
> > +       } else if ((*post_divider =3D=3D 13) || (*post_divider =3D=3D 1=
4)) {
...
> > +       ret =3D anx7625_write_and(ctx, ctx->i2c.rx_p1_client, MIPI_DIGI=
TAL_PLL_16,
> > +                               ~(REF_CLK_27000KHZ << MIPI_FREF_D_IND))=
;
> > +       ret |=3D anx7625_write_or(ctx, ctx->i2c.rx_p1_client, MIPI_DIGI=
TAL_PLL_16,
> > +                               (REF_CLK_27000KHZ << MIPI_FREF_D_IND));
> > +       /* post divider */
...
> > +       ret |=3D anx7625_write_and(ctx, ctx->i2c.rx_p1_client, MIPI_DIG=
ITAL_PLL_7,
...
> > +       ret =3D anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, PIXEL_CLO=
CK_L,
> > +                               (ctx->dt.pixelclock.min / 1000) & 0xFF)=
;
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, PIXEL_CL=
OCK_H,
...
> > +       /* HFP */
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p2_client,
> > +                       HORIZONTAL_FRONT_PORCH_L, ctx->dt.hfront_porch.=
min);
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p2_client,
...
> > +       /* HBP */
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p2_client,
> > +                       HORIZONTAL_BACK_PORCH_L, ctx->dt.hback_porch.mi=
n);
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p2_client,
> > +                       HORIZONTAL_BACK_PORCH_H, ctx->dt.hback_porch.mi=
n >> 8);
> > +       /* Vactive */
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p2_client, ACTIVE_L=
INES_L,
> > +                       ctx->dt.vactive.min);
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p2_client, ACTIVE_L=
INES_H,
...
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p1_client, MIPI_PLL=
_N_NUM_7_0,
> > +                       (n & 0xff));
> > +       /* diff */
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p1_client,
...
> > +       return anx7625_reg_write(ctx, ctx->i2c.rx_p1_client, MIPI_SWAP,=
 val);
> > +}
> > +
...
> > +
> > +       /*
> > +        * Decreased HS prepare timing delay from 160ns to 80ns work wi=
th
...
> > +        */
> > +       ret =3D anx7625_write_and(ctx, ctx->i2c.tx_p0_client, 0x87, 0xf=
e);
> > +       ret |=3D anx7625_write_and(ctx, ctx->i2c.tx_p2_client, 0x08, 0x=
7f);
> > +
...
> > +                              AUX_RST);
> > +       ret |=3D anx7625_write_and(ctx, ctx->i2c.tx_p2_client, RST_CTRL=
2,
> > +                                ~AUX_RST);
> > +       return ret;
> > +}
...
> > +       c =3D anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, AP_AUX_BUFF_=
START);
> > +       if (c < 0) {
...
> > +               if (ret) {
> > +                       sp_tx_rst_aux(ctx);
> > +                       DRM_DEV_DEBUG_DRIVER(dev, "edid read failed, re=
set!\n");
> > +               } else {
> > +                       ret =3D anx7625_reg_block_read(ctx, ctx->i2c.rx=
_p0_client,
> > +                                                    AP_AUX_BUFF_START,
> > +                                                    MAX_DPCD_BUFFER_SI=
ZE,
...
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p0_client,
> > +                                AP_AUX_CTRL_STATUS,
> > +                                AP_AUX_CTRL_ADDRONLY | AP_AUX_CTRL_OP_=
EN);
> > +
...
> > +               case 1:
> > +                       for (i =3D 0; i < 8; i++) {
> > +                               offset =3D (i + count * 8) * MAX_DPCD_B=
UFFER_SIZE;
...
> > +
> > +       /* reset main ocm */
> > +       ret =3D anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, 0x88, 0x4=
0);
> > +       /* Disable PD */
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p0_client,
> > +                                AP_AV_STATUS, AP_DISABLE_PD);
> > +       /* release main ocm */
> > +       ret |=3D anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, 0x88, 0x=
00);
> > +
> > +       if (ret < 0)
...
> > +                       ret =3D anx7625_reg_read(ctx, ctx->i2c.rx_p0_cl=
ient,
> > +                                              FLASH_LOAD_STA);
> > +                       if (ret < 0) {
...
> > +                               return;
> > +                       }
> > +                       if ((ret & FLASH_LOAD_STA_CHK) =3D=3D FLASH_LOA=
D_STA_CHK) {
...
> > +                                                        OCM_FW_REVERSI=
ON));
> > +                               DRM_DEV_DEBUG_DRIVER(dev, "Driver versi=
on %s\n",
> > +                                                    ANX7625_DRV_VERSIO=
N);
> > +
...
> > +       ret =3D devm_extcon_register_notifier(&ctx->client->dev, ctx->e=
xtcon,
> > +                                           EXTCON_DISP_DP, &ctx->event=
_nb);
> > +       if (ret) {
...
> > +       platform->pdata.gpio_p_on =3D
> > +               devm_gpiod_get_optional(dev, "enable", GPIOD_OUT_LOW);
> > +       /* gpio for chip reset */
> > +       platform->pdata.gpio_reset =3D
> > +               devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_LOW);
> > +
> > +       if (platform->pdata.gpio_p_on && platform->pdata.gpio_reset) {
> > +               platform->pdata.low_power_mode =3D 1;
> > +               DRM_DEV_DEBUG_DRIVER(dev, "low power mode, pon %d, rese=
t %d.\n",
> > +                                    desc_to_gpio(platform->pdata.gpio_=
p_on),
> > +                                    desc_to_gpio(platform->pdata.gpio_=
reset));
> > +       } else {
> > +               platform->pdata.low_power_mode =3D 0;
...
> > +
> > +       /* not support HDCP */
> > +       ret =3D anx7625_write_and(ctx, ctx->i2c.rx_p1_client, 0xee, 0x9=
f);
> > +
> > +       /* try auth flag */
> > +       ret |=3D anx7625_write_or(ctx, ctx->i2c.rx_p1_client, 0xec, 0x1=
0);
> > +       /* interrupt for DRM */
> > +       ret |=3D anx7625_write_or(ctx, ctx->i2c.rx_p1_client, 0xff, 0x0=
1);
> > +       if (ret < 0)
> > +               return;
> > +
...
> > +{
> > +       return anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, SYSTEM_STST=
US);
> > +}
> > +
...
ags);
> > +
> > +       if (pdata->panel_flags =3D=3D 1)
> > +               pdata->internal_panel =3D 1;
> > +       else if (pdata->panel_flags =3D=3D 2)
> > +               pdata->extcon_supported =3D 1;
> > +       DRM_DEV_DEBUG_DRIVER(dev, "%s support extcon, %s internal panel=
\n",
> > +                            pdata->extcon_supported ? "" : "not",
> > +                            pdata->internal_panel ? "has" : "no");
> > +
> > +       pdata->node.mipi_dsi_host_node =3D of_graph_get_remote_node(np,=
 0, 0);
> > +       if (pdata->node.mipi_dsi_host_node) {
> > +               of_node_put(pdata->node.mipi_dsi_host_node);
> > +               pdata->dsi_supported =3D 1;
> > +               DRM_DEV_DEBUG_DRIVER(dev, "found dsi host node.\n");
> > +       } else {
> > +               pdata->node.mipi_dpi_host_node =3D
...
pported);
> > +
> > +       pdata->node.panel_node =3D of_graph_get_port_by_id(np, 2);
> > +       if (pdata->node.panel_node) {
> > +               of_node_put(pdata->node.panel_node);
> > +               out_ep =3D of_get_child_by_name(pdata->node.panel_node,
> > +                                             "endpoint");
> > +               if (!out_ep) {
...
> > +               }
> > +
> > +               panel_node =3D of_graph_get_remote_port_parent(out_ep);
> > +               of_node_put(out_ep);
> > +
> > +               pdata->panel =3D of_drm_find_panel(panel_node);
...
> > +               return;
> > +
> > +       ret =3D drm_panel_unprepare(ctx->pdata.panel);
> > +       if (ret)
...
> > +               return;
> > +
> > +       ret =3D drm_panel_prepare(ctx->pdata.panel);
> > +       if (ret < 0)
...
> > +       mutex_lock(&ctx->lock);
> > +
> > +       if (ctx->pdata.panel && atomic_read(&ctx->panel_power) =3D=3D 0=
) {
> > +               turn_off_flag =3D 1;
...
> > +
> > +       p_edid->edid_block_num =3D sp_tx_edid_read(ctx, p_edid->edid_ra=
w_data);
> > +
> > +       err =3D -EIO;
...
> > +       if (ctx->pdata.panel && turn_off_flag =3D=3D 1)
...
> > +
> > +       if (ctx->pdata.dsi_supported)
> > +               mipi_host_node =3D ctx->pdata.node.mipi_dsi_host_node;
> > +       else
> > +               mipi_host_node =3D ctx->pdata.node.mipi_dpi_host_node;
> > +
> > +       if (!mipi_host_node) {
...
> > +       }
> > +
> > +       host =3D of_find_mipi_dsi_host_by_node(mipi_host_node);
> > +       if (!host) {
...
> > +
> > +       err =3D drm_connector_attach_encoder(&ctx->connector, bridge->e=
ncoder);
> > +       if (err) {
...
> > +                                   const struct drm_display_mode *old_=
mode,
> > +                                   const struct drm_display_mode *mode=
)
> > +{
...
ck.min);
> > +       DRM_DEV_DEBUG_DRIVER(dev, "hactive(%d), hsync(%d), hfp(%d), hbp=
(%d)\n",
> > +                            ctx->dt.hactive.min,
> > +                            ctx->dt.hsync_len.min,
> > +                            ctx->dt.hfront_porch.min,
> > +                            ctx->dt.hback_porch.min);
> > +       DRM_DEV_DEBUG_DRIVER(dev, "vactive(%d), vsync(%d), vfp(%d), vbp=
(%d)\n",
> > +                            ctx->dt.vactive.min,
...
> > +
> > +       if (ctx->pdata.panel) {
> > +               ret =3D drm_panel_enable(ctx->pdata.panel);
> > +               if (ret < 0) {
...
> > +
> > +       if (ctx->pdata.panel) {
> > +               ret =3D drm_panel_disable(ctx->pdata.panel);
> > +               if (ret < 0)
...
> > +       .detach =3D anx7625_bridge_detach,
> > +       .disable =3D anx7625_bridge_disable,
> > +       .post_disable =3D anx7625_post_disable,
> > +       .pre_enable =3D anx7625_pre_enable,
...
> > +
> > +       ret =3D anx7625_parse_dt(dev, pdata);
> > +       if (ret) {
...
> > +               ret =3D -EINVAL;
> > +               platform->extcon =3D extcon_get_edev_by_phandle(dev, 0)=
;
...
> > +               }
> > +
> > +               ret =3D devm_request_threaded_irq(dev, platform->pdata.=
intp_irq,
> > +                                               NULL, anx7625_intr_hpd_=
isr,
> > +                                               IRQF_TRIGGER_FALLING |
> > +                                               IRQF_ONESHOT,
> > +                                               "anx7625-intp", platfor=
m);
> > +               if (ret) {
...
> > +#define  HORIZONTAL_TOTAL_PIXELS_H    0x1A  /* note: bit[7:6] are rese=
rved */
> > +#define  HORIZONTAL_ACTIVE_PIXELS_L   0x1B
> > +#define  HORIZONTAL_ACTIVE_PIXELS_H   0x1C  /* note: bit[7:6] are rese=
rved */
> > +#define  HORIZONTAL_FRONT_PORCH_L     0x1D
> > +#define  HORIZONTAL_FRONT_PORCH_H     0x1E  /* note: bit[7:4] are rese=
rved */
> > +#define  HORIZONTAL_SYNC_WIDTH_L      0x1F
> > +#define  HORIZONTAL_SYNC_WIDTH_H      0x20  /* note: bit[7:4] are rese=
rved */
> > +#define  HORIZONTAL_BACK_PORCH_L      0x21
> > +#define  HORIZONTAL_BACK_PORCH_H      0x22  /* note: bit[7:4] are rese=
rved */
> > +
...
> > +#define  CLK_1MEG_RB     6  /* 1MHz clock reset; 0=3Dreset, 0=3Dreset =
release */
