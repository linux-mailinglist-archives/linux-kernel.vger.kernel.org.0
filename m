...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id DE16E14FE90
	for <lists+linux-kernel@lfdr.de>; Sun,  2 Feb 2020 18:18:04 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726971AbgBBRSD (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Sun, 2 Feb 2020 12:18:03 -0500
Received: from mail.kernel.org ([198.145.29.99]:54992 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726900AbgBBRSC (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Sun, 2 Feb 2020 12:18:02 -0500
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 5E79520679;
        Sun,  2 Feb 2020 17:17:57 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1580663879;
        bh=PGs60P4k3iKXOf8OiZfB14h6Cvb1yf6lxeNAW4Wv8v4=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=2R6kXI37MHbDAPriyC/dzK8l2p8qkAXV4abRUTP8NnnOy7uNGsRjXMNjaJND6hdba
         MSULh4CdOoyMEneyIEUaN09RERiDmRE5+YwdyoDytsXy7nMPLiX/M/cKHfkyEfnemn
         uyvxULWXVHSwDTLE6BgcZkM4g1UPa3yLvJEdqL14=
Date:   Sun, 2 Feb 2020 17:17:54 +0000
...
Message-ID: <20200202171754.78f53c55@archlinux>
...
On Tue, 21 Jan 2020 15:08:24 -0700
...
That's unusual as interfaces go and would normally make
people expect a value of 0...

Having had a look at the data sheet I think these differential
channels are actually an attempt to subtract the low pass
version of the same channel from the current version
(effectively a high pass filter).  Given there is
also a low pass filter on the raw input, it looks like
we really have some sort of bandpass filter.
The actual filter is horrible as you have different thresholds
for moving the average one way or the other.

So is it actually useful to output proximityX_raw at all?
They sound liable to be extremely messy for userspace to
deal with.  I would just report 4 simple non differential
channels. Ideally we'd have a generic description for that
filter, but its so 'weird' I'm not sure we'll be able to come
up with one.

The 4th sensor is interesting, as a combination of the other 3.
We currently have no way of representing that.  Hmm. This may
be a rare case for using extend_name to make it clear what it
is.
...
I'm fairly sure you can't get here with the trigger enabled.  You
have claimed direct mode.  That can't succeed if triggered buffer
capture is enabled.
...
Seems unlikely to me that this actually varies enough to make dynamic
resizing worth doing.  I would just put a suitably big fixed
size array in data in the first place.

There is a reasonable chance that will not need any more memory at all
and if it does it won't be worse than what you have here.
...
As this is obvious from the code, the comment is just something that might
rot in the future. I'd rather we had no comment on this one.
...
You should also verify that this matches the expected device type.
By all means just give a warning if it doesn't and carry on though.
...
> +
> +	kfree(data->buffer);
Hmm. This results in us removing the buffer before the interfaces
that might lead to it being used are unregistered. 
...
I'm getting more paranoid on these.  Does semtech have a public
list of what they have allocated somewhere? 
I guess you have these live in a firmware though so we'll need
...
No point in specifying the data field if they are all going to be 0.
