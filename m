...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 4DDE2DF273
	for <lists+linux-kernel@lfdr.de>; Mon, 21 Oct 2019 18:07:59 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729874AbfJUQHr (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 21 Oct 2019 12:07:47 -0400
Received: from us-smtp-delivery-1.mimecast.com ([207.211.31.120]:51296 "EHLO
        us-smtp-1.mimecast.com" rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org
        with ESMTP id S1729666AbfJUQHq (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 21 Oct 2019 12:07:46 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
        s=mimecast20190719; t=1571674064;
        h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
         to:to:cc:cc:mime-version:mime-version:content-type:content-type:
         content-transfer-encoding:content-transfer-encoding:
         in-reply-to:in-reply-to:references:references;
        bh=2GZamw6lVYzrGVF/6j3ZX//5vuWGtKtzVPr9TeudlUA=;
        b=E+r51CA9lz/bN1vqWMgre14q6Y2sQ/dQIUQqjk6erre8SDfZqKGdoY/TpUAu/grlQwFuG4
        BcLuqT1egFZ/o9bZ9LZl9/ROwYb7eJzJ4nbRuLdAO4/pog2BcpCLuBzRHuhSuFrltPhIof
        mNnogBC1G4vqBj4qdO1QFAU5juVy27w=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-177-3BpvP4fYPc2YBqT0_S9-Iw-1; Mon, 21 Oct 2019 12:07:42 -0400
Received: from smtp.corp.redhat.com (int-mx03.intmail.prod.int.phx2.redhat.com [10.5.11.13])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mimecast-mx01.redhat.com (Postfix) with ESMTPS id B064680183E;
        Mon, 21 Oct 2019 16:07:40 +0000 (UTC)
Received: from krava (unknown [10.43.17.61])
        by smtp.corp.redhat.com (Postfix) with SMTP id 9265360606;
        Mon, 21 Oct 2019 16:07:38 +0000 (UTC)
Date:   Mon, 21 Oct 2019 18:07:37 +0200
...
Subject: Re: [PATCH v2 3/5] perf report: Sort by sampled cycles percent per
 block for stdio
Message-ID: <20191021160737.GG32718@krava>
...
X-MC-Unique: 3BpvP4fYPc2YBqT0_S9-Iw-1
...
> +
> +enum {
> +=09PERF_HPP_REPORT__BLOCK_TOTAL_CYCLES_COV,
...
> +=09[PERF_HPP_REPORT__BLOCK_TOTAL_CYCLES_COV] =3D {
...
> @@ -277,7 +332,8 @@ static int process_sample_event(struct perf_tool *too=
l,
...
> @@ -290,9 +346,10 @@ static int process_sample_event(struct perf_tool *to=
ol,
>  =09if (al.map !=3D NULL)
...
> @@ -480,6 +537,349 @@ static size_t hists__fprintf_nr_sample_events(struc=
t hists *hists, struct report
...
> +{
> +=09struct block_fmt *block_fmt =3D container_of(fmt, struct block_fmt, f=
mt);
> +
...
> +
> +=09return scnprintf(hpp->buf, hpp->size, "%*s", block_fmt->width, buf);
> +}
> +
...
> +
> +=09if (rep->cycles_count) {
> +=09=09l =3D ((double)bi_l->cycles / (double)rep->cycles_count) * 1000.0;
> +=09=09r =3D ((double)bi_r->cycles / (double)rep->cycles_count) * 1000.0;
...
> +}
> +
> +static int block_range_entry(struct perf_hpp_fmt *fmt, struct perf_hpp *=
hpp,
...
> +
> +=09start_line =3D map__srcline(he->ms.map, bi->sym->start + bi->start,
> +=09=09=09=09  he->ms.sym);
> +
> +=09end_line =3D map__srcline(he->ms.map, bi->sym->start + bi->end,
> +=09=09=09=09he->ms.sym);
> +
> +=09if ((start_line !=3D SRCLINE_UNKNOWN) && (end_line !=3D SRCLINE_UNKNO=
WN)) {
...
> +}
> +
> +static int block_dso_entry(struct perf_hpp_fmt *fmt, struct perf_hpp *hp=
p,
> +=09=09=09   struct hist_entry *he)
> +{
> +=09struct block_fmt *block_fmt =3D container_of(fmt, struct block_fmt, f=
...
> +
> +=09switch (idx) {
> +=09case PERF_HPP_REPORT__BLOCK_TOTAL_CYCLES_COV:
> +=09=09fmt->entry =3D block_cycles_cov_entry;
> +=09=09fmt->cmp =3D block_info__cmp;
> +=09=09fmt->sort =3D block_cycles_cov_sort;
> +=09=09break;
...
> +=09=09&block_fmts[PERF_HPP_REPORT__BLOCK_TOTAL_CYCLES_COV].fmt);
> +}
> +
> +static void init_block_info(struct block_info *bi, struct symbol *sym,
> +=09=09=09    struct cyc_hist *ch, int offset,
> +=09=09=09    u64 total_cycles)
> +{
> +=09bi->sym =3D sym;
> +=09bi->start =3D ch->start;
> +=09bi->end =3D offset;
> +=09bi->cycles =3D ch->cycles;
> +=09bi->cycles_aggr =3D ch->cycles_aggr;
> +=09bi->num =3D ch->num;
> +=09bi->num_aggr =3D ch->num_aggr;
> +=09bi->total_cycles =3D total_cycles;
> +}
> +
> +static int add_block_per_sym(struct hist_entry *he, struct block_hist *b=
h,
> +=09=09=09     u64 *block_cycles, u64 total_cycles)
> +{
> +=09struct annotation *notes;
> +=09struct cyc_hist *ch;
> +=09static struct addr_location al;
> +=09u64 cycles =3D 0;
> +
> +=09if (!he->ms.map || !he->ms.sym)
> +=09=09return 0;
> +
> +=09memset(&al, 0, sizeof(al));
> +=09al.map =3D he->ms.map;
> +=09al.sym =3D he->ms.sym;
> +
> +=09notes =3D symbol__annotation(he->ms.sym);
> +=09if (!notes || !notes->src || !notes->src->cycles_hist)
> +=09=09return 0;
> +=09ch =3D notes->src->cycles_hist;
> +=09for (unsigned int i =3D 0; i < symbol__size(he->ms.sym); i++) {
> +=09=09if (ch[i].num_aggr) {
> +=09=09=09struct block_info *bi;
> +=09=09=09struct hist_entry *he_block;
> +
> +=09=09=09bi =3D block_info__new();
> +=09=09=09if (!bi)
> +=09=09=09=09return -1;
> +
> +=09=09=09init_block_info(bi, he->ms.sym, &ch[i], i,
> +=09=09=09=09=09total_cycles);
...
> +=09while (next) {
> +=09=09he =3D rb_entry(next, struct hist_entry, rb_node);
> +=09=09add_block_per_sym(he, bh, &rep->block_cycles,
> +=09=09=09=09  rep->cycles_count);
> +=09=09next =3D rb_next(&he->rb_node);
> +=09}
> +
> +=09hists__output_resort_cb(&bh->block_hists, NULL, resort_cb);
> +=09hists__clear_filtered(&bh->block_hists);
> +}
> +
...
> @@ -500,6 +900,12 @@ static int perf_evlist__tty_browse_hists(struct evli=
st *evlist,
>  =09=09=09continue;
> =20
...
> +=09=09=09pr_err("Error: -s total_cycles can be only used together with -=
-stdio\n");
> +=09=09=09goto error;
> +=09=09}
> +=09}
...
> +=09if (ui__has_annotation() || report.symbol_ipc || report.total_cycles)=
 {
...
> @@ -558,6 +558,25 @@ static int hist_entry__block_fprintf(struct hist_ent=
ry *he,
>  =09return ret;
>  }
> =20
...
>  =09u64=09=09=09end;
>  =09u64=09=09=09cycles;
>  =09u64=09=09=09cycles_aggr;
> +=09u64=09=09=09total_cycles;
>  =09s64=09=09=09cycles_spark[NUM_SPARKS];
>  =09int=09=09=09num;
>  =09int=09=09=09num_aggr;
> diff --git a/tools/perf/util/hist.c b/tools/perf/util/hist.c
> index af65ce950ba2..521f7185a94f 100644
> --- a/tools/perf/util/hist.c
> +++ b/tools/perf/util/hist.c
> @@ -756,6 +756,10 @@ struct hist_entry *hists__add_entry_block(struct his=
ts *hists,
