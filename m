...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 4EF8817CE2D
	for <lists+linux-kernel@lfdr.de>; Sat,  7 Mar 2020 13:40:43 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726296AbgCGMkl (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Sat, 7 Mar 2020 07:40:41 -0500
Received: from mail.kernel.org ([198.145.29.99]:58734 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726139AbgCGMkl (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Sat, 7 Mar 2020 07:40:41 -0500
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 54868206D7;
        Sat,  7 Mar 2020 12:40:36 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1583584838;
        bh=LTpEjUztTBpQsrCiZp5VroKdnR7yTqQS7QB0N32lsvc=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=DyK+0K/xOlfFxlguGUEcwtEBA5/y/sUVYN/VaiAjBHwjiUwI750DXSf7LniY9Tvp0
         UE038IYVWVKOucSczjWfUIAxHQeL99JzuvWf2rlZoTRlyHEMeta8oCLW0jO++Lkwcs
         ehox+QSpBX4vg+ji/GUaxOPaGjDRKVb8EyyPTI6k=
Date:   Sat, 7 Mar 2020 12:40:33 +0000
...
Subject: Re: [PATCH v4] iio: Add SEMTECH SX9310/9311 sensor driver
Message-ID: <20200307124033.5007c5df@archlinux>
In-Reply-To: <20200226121020.1.I1f56fe698017f22d6e825c913c256d5afc2ad69f@changeid>
References: <20200226121020.1.I1f56fe698017f22d6e825c913c256d5afc2ad69f@changeid>
...
On Wed, 26 Feb 2020 12:13:04 -0700
...
I previously missed the unusual string matching games you are
playing to check the device type.  Given we can just put the
WHOAMI or an enum value in the data for the relevant ACPI / DT device
table it is much easier to just do that and avoid matching strings
when we've effectively already done that to load the driver in the
first place.

A few more small bits inline.  Apologies for delay in review, I'm a bit
behind at the moment, but hopefully catch up today.
...
Hmm. Not the prettiest bit of interface, but I can't see a better way :)
...
> new file mode 100644
> index 00000000000000..40d6f32903cf36
> --- /dev/null
...
If you make the change I suggest below to actually use the data field rather
than string match on these, then you can just put these IDs in line in the
relevant tables.
...
buffer gets used directly in an iio_push_to_buffer_with_timestamp call
That puts an extra demand on the buffer provided - it has to have room for
the timestamp to be added on the end.
Hence the buffer needs to have room for an aligned u64 after the channel data.

It's a somewhat horrible interface but it saves a copy of the data in the fast
path.
...
Hmm. This sort of fits better in update_scan_mode, but I can see
why you did this here so that the matching code would be in the postdisable
callback.  We don't call update_scan_mode in the disable path
(on basis it doesn't make sense for many devices).
...
Sorry, I'd not noticed this before, but all the string comparison in here seems
way more complex than it needs to be.   Why not just use an enum value
in the data fields of ACPI id table and dt one.  (or possibly just put
the whoami values in there).  That way you'll be able to verify if they
match without having to resort to comparing names.
