...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 8A04212E8D4
	for <lists+linux-kernel@lfdr.de>; Thu,  2 Jan 2020 17:42:33 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728848AbgABQm3 convert rfc822-to-8bit (ORCPT
        <rfc822;lists+linux-kernel@lfdr.de>); Thu, 2 Jan 2020 11:42:29 -0500
Received: from pegase1.c-s.fr ([93.17.236.30]:64542 "EHLO pegase1.c-s.fr"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1728280AbgABQm3 (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Thu, 2 Jan 2020 11:42:29 -0500
...
>> +++ b/arch/powerpc/include/asm/book3s/64/pgalloc.h
>> @@ -87,9 +87,9 @@ static inline void pgd_free(struct mm_struct *mm,  
>> pgd_t *pgd)
...
>> pud_t *pud)
>> +static inline void p4d_populate(struct mm_struct *mm, p4d_t *pgd,  
>> pud_t *pud)
>>  {
...
>> @@ -949,54 +949,60 @@ static inline bool pud_access_permitted(pud_t  
>> pud, bool write)
...
>> +++ b/arch/powerpc/kvm/book3s_64_mmu_radix.c
>> @@ -496,17 +496,39 @@ static void kvmppc_unmap_free_pud(struct kvm  
>> *kvm, pud_t *pud,
>>  	pud_free(kvm->mm, pud);
>>  }
>>
...
>> +}
>> +
>>  void kvmppc_free_pgtable_radix(struct kvm *kvm, pgd_t *pgd,  
>> unsigned int lpid)
>>  {
>>  	unsigned long ig;
>>
...
>>  	}
>>  }
>> @@ -568,6 +590,7 @@ int kvmppc_create_pte(struct kvm *kvm, pgd_t  
>> *pgtable, pte_t pte,
...
>> @@ -575,9 +598,15 @@ int kvmppc_create_pte(struct kvm *kvm, pgd_t  
>> *pgtable, pte_t pte,
>>
...
>>
>> @@ -599,12 +628,18 @@ int kvmppc_create_pte(struct kvm *kvm, pgd_t  
>> *pgtable, pte_t pte,
...
>>
>> @@ -1222,6 +1257,7 @@ static ssize_t debugfs_radix_read(struct file  
...
>>  	pmd_t pmd, *pmdp;
>>  	pte_t *ptep;
>> @@ -1300,7 +1336,14 @@ static ssize_t debugfs_radix_read(struct  
...
>> @@ -87,7 +87,7 @@ static void flush_range(struct mm_struct *mm,  
>> unsigned long start,
>>  	if (start >= end)
>>  		return;
>>  	end = (end - 1) | ~PAGE_MASK;
>> -	pmd = pmd_offset(pud_offset(pgd_offset(mm, start), start), start);
>> +	pmd = pmd_offset(pud_offset(p4d_offset(pgd_offset(mm, start),  
>> start), start), start);
>>  	for (;;) {
>>  		pmd_end = ((start + PGDIR_SIZE) & PGDIR_MASK) - 1;
>>  		if (pmd_end > end)
>> @@ -145,7 +145,7 @@ void flush_tlb_page(struct vm_area_struct *vma,  
>> unsigned long vmaddr)
>>  		return;
>>  	}
>>  	mm = (vmaddr < TASK_SIZE)? vma->vm_mm: &init_mm;
>> -	pmd = pmd_offset(pud_offset(pgd_offset(mm, vmaddr), vmaddr), vmaddr);
>> +	pmd = pmd_offset(pud_offset(p4d_offset(pgd_offset(mm, vmaddr),  
>> vmaddr), vmaddr), vmaddr);
>>  	if (!pmd_none(*pmd))
...
>>  int hash__map_kernel_page(unsigned long ea, unsigned long pa,  
>> pgprot_t prot)
>>  {
>>  	pgd_t *pgdp;
>> +	p4d_t *p4dp;
...
>>  	pmd_t *pmdp;
>>  	pte_t *ptep;
>> @@ -155,7 +156,8 @@ int hash__map_kernel_page(unsigned long ea,  
...
>> +++ b/arch/powerpc/mm/book3s64/radix_pgtable.c
>> @@ -64,17 +64,24 @@ static int early_map_kernel_page(unsigned long  
>> ea, unsigned long pa,
>>  {
...
>>  		ptep = (pte_t *)pudp;
>>  		goto set_the_pte;
>> @@ -114,6 +121,7 @@ static int __map_kernel_page(unsigned long ea,  
>> unsigned long pa,
>>  {
>>  	unsigned long pfn = pa >> PAGE_SHIFT;
>>  	pgd_t *pgdp;
...
>>  	pmd_t *pmdp;
>>  	pte_t *ptep;
>> @@ -136,7 +144,8 @@ static int __map_kernel_page(unsigned long ea,  
>> unsigned long pa,
>>  	 * boot.
>>  	 */
>>  	pgdp = pgd_offset_k(ea);
...
>>  		return -ENOMEM;
>>  	if (map_page_size == PUD_SIZE) {
>> @@ -173,6 +182,7 @@ void radix__change_memory_range(unsigned long  
...
>>  	pmd_t *pmdp;
>>  	pte_t *ptep;
>> @@ -185,7 +195,8 @@ void radix__change_memory_range(unsigned long  
>> start, unsigned long end,
>>
...
>> +++ b/arch/powerpc/mm/book3s64/subpage_prot.c
>> @@ -54,6 +54,7 @@ static void hpte_flush_range(struct mm_struct  
...
>>  	pmd_t *pmd;
>>  	pte_t *pte;
>> @@ -62,7 +63,10 @@ static void hpte_flush_range(struct mm_struct  
>> *mm, unsigned long addr,
...
>> @@ -112,6 +112,7 @@ static int __hugepte_alloc(struct mm_struct  
>> *mm, hugepd_t *hpdp,
...
>>  	pmd_t *pm;
>>  	hugepd_t *hpdp = NULL;
>> @@ -121,20 +122,21 @@ pte_t *huge_pte_alloc(struct mm_struct *mm,  
...
>>  			return NULL;
>>  		if (pshift == PUD_SHIFT)
>> @@ -159,10 +161,10 @@ pte_t *huge_pte_alloc(struct mm_struct *mm,  
>> unsigned long addr, unsigned long sz
>>  #else
>>  	if (pshift >= PGDIR_SHIFT) {
>>  		ptl = &mm->page_table_lock;
...
>>  			return NULL;
>>  		if (pshift >= PUD_SHIFT) {
>> @@ -384,7 +386,7 @@ static void hugetlb_free_pmd_range(struct  
...
>>  {
>> @@ -394,7 +396,7 @@ static void hugetlb_free_pud_range(struct  
>> mmu_gather *tlb, pgd_t *pgd,
>>
>>  	start = addr;
>>  	do {
>> -		pud = pud_offset(pgd, addr);
...
>> @@ -429,8 +431,8 @@ static void hugetlb_free_pud_range(struct  
>> mmu_gather *tlb, pgd_t *pgd,
>>  	if (end - 1 > ceiling - 1)
>>  		return;
>>
...
>> +++ b/arch/powerpc/mm/kasan/kasan_init_32.c
>> @@ -36,7 +36,7 @@ static int __ref  
>> kasan_init_shadow_page_tables(unsigned long k_start, unsigned l
>>  	unsigned long k_cur, k_next;
>>  	pgprot_t prot = slab_is_available() ? kasan_prot_ro() : PAGE_KERNEL;
>>
>> -	pmd = pmd_offset(pud_offset(pgd_offset_k(k_start), k_start), k_start);
>> +	pmd = pmd_offset(pud_offset(p4d_offset(pgd_offset_k(k_start),  
>> k_start), k_start), k_start);
>>
>>  	for (k_cur = k_start; k_cur != k_end; k_cur = k_next, pmd++) {
>>  		pte_t *new;
>> @@ -94,7 +94,7 @@ static int __ref kasan_init_region(void *start,  
>> size_t size)
...
>> +		pmd_t *pmd =  
>> pmd_offset(pud_offset(p4d_offset(pgd_offset_k(k_cur), k_cur),  
>> k_cur), k_cur);
...
>> +	pmd_t *pmd = pmd_offset(pud_offset(p4d_offset(pgd_offset_k(addr),  
>> addr), addr), addr);
>>
...
>> @@ -104,7 +104,7 @@ unsigned long __init mmu_mapin_ram(unsigned  
>> long base, unsigned long top)
>>  		pmd_t *pmdp;
...
>>  		*pmdp++ = __pmd(val);
>>  		*pmdp++ = __pmd(val);
>> @@ -119,7 +119,7 @@ unsigned long __init mmu_mapin_ram(unsigned  
>> long base, unsigned long top)
>>  		pmd_t *pmdp;
...
>> +++ b/arch/powerpc/mm/nohash/book3e_pgtable.c
>> @@ -73,6 +73,7 @@ static void __init *early_alloc_pgtable(unsigned  
>> long size)
>>  int __ref map_kernel_page(unsigned long ea, unsigned long pa,  
>> pgprot_t prot)
>>  {
>>  	pgd_t *pgdp;
>> +	p4d_t *p4dp;
...
>>  	pmd_t *pmdp;
>>  	pte_t *ptep;
>> @@ -80,7 +81,10 @@ int __ref map_kernel_page(unsigned long ea,  
>> unsigned long pa, pgprot_t prot)
...
>>  			return -ENOMEM;
>>  		pmdp = pmd_alloc(&init_mm, pudp, ea);
>> @@ -91,13 +95,16 @@ int __ref map_kernel_page(unsigned long ea,  
>> unsigned long pa, pgprot_t prot)
>>  			return -ENOMEM;
>>  	} else {
>>  		pgdp = pgd_offset_k(ea);
...
>> @@ -265,6 +265,7 @@ int huge_ptep_set_access_flags(struct  
>> vm_area_struct *vma,
...
>>  	pmd_t *pmd;
>>
>> @@ -272,7 +273,9 @@ void assert_pte_locked(struct mm_struct *mm,  
>> unsigned long addr)
>>  		return;
...
>>  	pmd_t pmd, *pmdp;
>>  	pte_t *ret_pte;
>> @@ -346,13 +350,30 @@ pte_t *__find_linux_pte(pgd_t *pgdir,  
>> unsigned long ea,
>>  		goto out_huge;
>>  	}
>>
...
>> @@ -63,7 +63,7 @@ int __ref map_kernel_page(unsigned long va,  
...
>>          pmd_t	*pmd;
>>          pte_t	*pte;
>> @@ -137,17 +138,20 @@ get_pteptr(struct mm_struct *mm, unsigned  
>> long addr, pte_t **ptep, pmd_t **pmdp)
>>
...
>> +++ b/arch/powerpc/mm/ptdump/hashpagetable.c
>> @@ -417,9 +417,9 @@ static void walk_pmd(struct pg_state *st, pud_t  
...
>> +++ b/arch/powerpc/mm/ptdump/ptdump.c
>> @@ -273,9 +273,9 @@ static void walk_pmd(struct pg_state *st, pud_t  
>> *pud, unsigned long start)
>>  	}
>>  }
>>
...
>>  	unsigned int i;
>>
>> @@ -289,6 +289,22 @@ static void walk_pud(struct pg_state *st,  
>> pgd_t *pgd, unsigned long start)
>>  	}
>>  }
>>
