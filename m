...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 1AAF510AEBD
	for <lists+linux-kernel@lfdr.de>; Wed, 27 Nov 2019 12:33:03 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726655AbfK0LdB (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Wed, 27 Nov 2019 06:33:01 -0500
Received: from mail1.bemta26.messagelabs.com ([85.158.142.116]:40102 "EHLO
        mail1.bemta26.messagelabs.com" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1726526AbfK0LdB (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Wed, 27 Nov 2019 06:33:01 -0500
Received: from [85.158.142.201] (using TLSv1.2 with cipher DHE-RSA-AES256-GCM-SHA384 (256 bits))
        by server-5.bemta.az-b.eu-central-1.aws.symcld.net id CF/72-19907-8EE5EDD5; Wed, 27 Nov 2019 11:32:56 +0000
...
Received: from mail-he1eur01lp2050.outbound.protection.outlook.com (HELO EUR01-HE1-obe.outbound.protection.outlook.com) (104.47.0.50)
  by server-24.tower-246.messagelabs.com with ECDHE-RSA-AES256-GCM-SHA384 encrypted SMTP; 27 Nov 2019 11:32:56 -0000
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 27 Nov 2019 11:32:54.4070
 (UTC)
...
>
> > Ok, what's being proposed here is for the codec to automatically contro=
l the PLL
> > rather than leaving it to the machine driver as is the case right now. =
In the
> > possible scenario where this is done using a card level widget to enabl=
e/disable
>
...
Yes, but by default the assumption is the codec will do things automaticall=
y
until the machine driver calls the relevant PLL config code.....

>
> > the PLL we will conflict with that using the current suggested approach=
 for the
> > da7213 driver, albeit with no real consequence other than configuring t=
he PLL
> > twice the first time a stream is started. It's a case of how to determi=
ne who's
...
called prior to the DAPM paths being sequenced. If a dedicated machine driv=
er
were to want to control the PLL via a DAPM widget at the card level (which =
is
done for other codecs for example on Intel platforms), the code in the
bias_level() function of the codec to auto configure the PLL would always b=
e
called the very first time a path is enabled before the relevant DAPM widge=
t for
the card is called, so you'd have two configurations of the PLL in successi=
on. I
don't expect that would cause issues, but it's not ideal. The other approac=
h
would be to have something in the machine driver like a dai_link init funct=
ion
to default the PLL config using the codec's PLL function which then prevent=
s
any chance of auto configuration subsequently. However that requires the pe=
rson
...
seems messy. Regarding the SYSCLK approach you mention, I'm not clear how t=
hat
