...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 2C7D6683E1
	for <lists+linux-kernel@lfdr.de>; Mon, 15 Jul 2019 09:09:21 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729162AbfGOHJL (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 15 Jul 2019 03:09:11 -0400
Received: from mail-lj1-f172.google.com ([209.85.208.172]:35855 "EHLO
        mail-lj1-f172.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726748AbfGOHJK (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 15 Jul 2019 03:09:10 -0400
Received: by mail-lj1-f172.google.com with SMTP id i21so15043642ljj.3
        for <linux-kernel@vger.kernel.org>; Mon, 15 Jul 2019 00:08:47 -0700 (PDT)
...
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=0dfBdAoAN2LBj5Mx8xBo2o2YVSiqNOsI58+hrNH2I3c=;
        b=
X-Gm-Message-State: APjAAAW8dYg2Axrxgwn9o6bpiB5Uu2Au2jq3rwo7UMTDQARCHJrSiul+
        OuaRZQTVJvFrd5NjUbGuJJLWPg7OFCyvqUrNvX8=
...
> The following changes since commit 6116b892bd4fd0ddc5f30566a556218bb2e1a9=
b6:
>
...
>       drm/amd/display: add fast_validate parameter to dcn20_validate_band=
width
...
>       drm/i915: Disable preemption and sleeping while using the punit sid=
eband
...
>       drm/i915: Enable render context support for gen4 (Broadwater to Can=
tiga)
...
>       drm/i915: Complete both freed-object passes before draining the wor=
kqueue
...
>       drm/i915: Assert breadcrumbs are correctly ordered in the signal ha=
ndler
...
>       drm/i915: Only reschedule the submission tasklet if preemption is p=
ossible
...
>       drm/i915: Extend I915_CONTEXT_PARAM_SSEU to support local ctx->engi=
ne[]
...
>       drm/i915/gtt: Neuter the deferred unbind callback from gen6_ppgtt_c=
leanup
...
>       drm/i915: Move GEM object domain management from struct_mutex to lo=
cal
...
e
>       drm/i915: Make default value for i915.mmio_debug a compile time opt=
ion
...
>       dma-buf: Discard old fence_excl on retrying get_fences_rcu for real=
loc
...
>       drm/i915: remove DRM_AUTH from IOCTLs which also have DRM_RENDER_AL=
LOW
...
>       drm/bridge: sii902x: re-order conditions to prevent out of bounds r=
ead
...
>       drm/i915: make enable/disable rpm assert function use the rpm struc=
ture
...
>       drm/bridge/synopsys: dw-hdmi: Fix unwedge crash when no pinctrl ent=
ries
...
>       drm/v3d: Drop reservation of a shared slot in the dma-buf reservati=
ons.
>       drm/v3d: Add missing implicit synchronization.
>       drm/doc: Allow new UAPI to be used once it's in drm-next/drm-misc-n=
ext.
...
>       drm/amd/powerplay: support ppfeatures sysfs interface on sw smu rou=
tine
...
>       drm/amd/powerplay: support sw smu hotspot and memory temperature re=
trieval
...
>       drm/i915/selftests: Check that gpu reset is usable from atomic cont=
ext
>
> Flora Cui (1):
...
>       vga_switcheroo: no need to check return value of debugfs_create fun=
ctions
>       panel: rocktech: no need to check return value of debugfs_create fu=
nctions
...
>       msm: adreno: no need to check return value of debugfs_create functi=
ons
...
>       drm/amdgpu: add gfx10 specific new member pa_sc_tile_steering_overr=
ide
...
>       drm/amdgpu/psp: start rlc autoload after psp recieved all gfx firmw=
are
>       drm/amdgpu/psp: switch to use sos_offset_bytes member as sys_bin_si=
ze
>       drm/amdgpu/psp: perform tmr_init and asd_init after loading sysdrv/=
sos
...
>       drm/amdgpu: add pa_sc_tile_steering_override to drm_amdgpu_info_dev=
ice
...
>       drm/i915: extract intel_gmbus.h from i915_drv.h and rename intel_i2=
c.c
...
>       drm/i915/bios: use port info child pointer to determine LSPCON pres=
ence
...
>
> Jordan Crouse (7):
>       drm/msm/adreno: Enable 64 bit mode by default on a5xx and a6xx targ=
ets
...
>       drm/amd/display: Program VTG params after programming Global Sync f=
or DCN2
...
>       drm/amd/powerplay: enable gfxclk ds,dcefclk ds and fw dstate on nav=
i10
...
>
> Kevin Wang (62):
>       drm/amd/powerplay: add helper function to get smu firmware & if ver=
sion
...
10
>       drm/amd/powerplay: add function display_configuration_changed for n=
avi10
...
>       drm/amd/powerplay: add function get_current_activity_percent for na=
vi10
...
> for navi10
>       drm/amd/powerplay: add function set_watermarks_table function for n=
avi10
>       drm/amd/powerplay: add function read_sensor for navi10
>       drm/amd/powerplay: fix dpm freq unit error (10KHz -> Mhz)
>       drm/amd/powerplay: simplify the interface of get_current_activity_p=
ercent
...
>       drm/amd/powerplay: move function thermal_get_temperature to veag20_=
ppt
...
>       drm/amd/powerplay: use pp_feature_mask to control uclk(mclk) dpm en=
abled
...
e
>       dt-bindings: display: bridge: thc63lvd1024: Document dual-link oper=
ation
...
>       drm: rcar-du: Skip LVDS1 output on Gen3 when using dual-link LVDS m=
ode
...
>
> Le.Ma (3):
>       drm/amdgpu: add structures for buffer allocate/release for rlc auto=
load
...
>       drm/i915/dmc: extract fw_info and table walk from intel_package_hea=
der
...
>       drm/mali: Convert to using __drm_atomic_helper_crtc_reset() for res=
et.
>       drm/rockchip: Convert to using __drm_atomic_helper_crtc_reset() for=
 reset.
>       drm/tegra: Convert to using __drm_atomic_helper_crtc_reset() for re=
set.
>       drm/msm: Convert to using __drm_atomic_helper_crtc_reset() for rese=
t.
>       drm/vkms: Convert to using __drm_atomic_helper_crtc_reset() for res=
et.
...
>       Merge remote-tracking branch 'drm/drm-next' into drm-misc-next-fixe=
s
...
>       drm/amdgpu: don't invalidate caches in RELEASE_MEM, only do the wri=
teback
>
> Marek Vasut (2):
...
>
> Maxime Ripard (21):
>       drm/rockchip: Change the scl_vop_cal_scl_fac to pass drm_format_inf=
o
>       drm: Remove users of drm_format_num_planes
>       drm: Remove users of drm_format_(horz|vert)_chroma_subsampling
>       drm/fourcc: Pass the format_info pointer to drm_format_plane_cpp
>       drm/fourcc: Pass the format_info pointer to drm_format_plane_width/=
height
...
>       drm/modes: Allow to specify rotation and reflection on the commandl=
ine
...
>       drm/amd/powerplay: Zero initialize freq in smu_v11_0_get_current_cl=
k_freq
...
>       drm/amd/display: Disable cursor when offscreen in negative directio=
n
>       drm/amd/display: Hook up CRC capture support for dce120
>       drm/amd/display: Explicitly specify update type per plane info chan=
ge
>       drm/amd/display: Switch the custom "max bpc" property to the DRM pr=
op
...
>       drm/amd/display: Expose HDR output metadata for supported connector=
s
>       drm/amd/display: Only force modesets when toggling HDR
>       drm/amd/display: Don't set mode_changed=3Dfalse if the stream was r=
emoved
...
>       drm/amd/display: Use current connector state if NULL when checking =
bpc
...
>       drm/amdkfd: Initialize HSA_CAP_ATS_PRESENT capability in topology c=
odes
...
>
> Oscar Mateo (2):
>       drm/i915/guc: Create vfuncs for the GuC interrupts control function=
s
...
>       drm/armada: move plane address and pitch calculation to atomic_chec=
k
...
>
> Samson Tam (3):
>       drm/amd/display: block passive dongle EDID Emulation for USB-C port=
s
...
>
> Sandor Yu (1):
>       drm/rockchip: cdn-dp: correct rate in the struct drm_dp_link assign=
ment
>
> Sean Paul (36):
...
>       drm/connector: Fix kerneldoc warning in HDR_OUTPUT_METADATA descrip=
tion
...
>       drm/msm/dsi: Use the new setup_encoder function in attach_dsi_devic=
e
...
>       drm/amd/display: fix crash on setmode when mode is close to bw limi=
t
...
>
> Tao.Huang (1):
>       drm/amd/display: fix resource saving missing when power state switc=
h
>
> Thierry Reding (6):
...
>       drm/ttm: Allow the driver to provide the ttm struct vm_operations_s=
truct
>       drm/ttm: TTM fault handler helpers
>       drm/vmwgfx: Implement an infrastructure for write-coherent resource=
s
...
>       drm: Add |struct drm_gem_vram_object| callbacks for |struct ttm_bo_=
driver|
>       drm: Add |struct drm_gem_vram_object| callbacks for |struct drm_dri=
ver|
...
>       drm/vboxvideo: Convert vboxvideo driver to |struct drm_gem_vram_obj=
ect|
>       drm/vboxvideo: Convert vboxvideo driver to VRAM MM
>       drm/hisilicon: Convert hibmc-drm driver to |struct drm_gem_vram_obj=
ect|
...
>       drm: Remove functions with kmap-object argument from GEM VRAM helpe=
rs
>
> Tianci Yin (2):
...
>       drm/bridge: tc358767: move video stream setup to tc_main_link_strea=
m
>       drm/bridge: tc358767: split stream enable/disable
>       drm/bridge: tc358767: move PXL PLL enable/disable to stream enable/=
disable
>       drm/bridge: tc358767: add link disable function
>       drm/bridge: tc358767: disable only video stream in tc_stream_disabl=
e
...
>       drm/bridge: tc358767: copy the mode data, instead of storing the po=
inter
...
>
> Tony Cheng (1):
>       drm/amd/display: move dsc clock from plane_resource to stream_resou=
rce
>
> Trigger Huang (11):
...
>       drm/i915: Convert i915_reg_read_ioctl to use explicit mmio accessor=
s
...
>       drm/i915/guc: Move intel_guc_reserved_gtt_size to intel_wopcm_guc_s=
ize
...
>       drm/i915: Bump fb stride limit to 128KiB for gen4+ and 256KiB for g=
en7+
>       drm/i915: Bump gen7+ fb size limits to 16kx16k
>       drm: Add HLG EOTF
>       drm/i915: Make sandybridge_pcode_read() deal with the second data r=
egister
...
>       drm/i915: Move intel_dp->prepare_link_train assignment into ddi cod=
e
...
>       drm/i915/sdvo: Actually print the reason why the SDVO command faile=
d
...
>       drm/i915: Add missing commas to the end of the subplatform ID array=
s
...
>
> Vivek Gautam (1):
>       drm/panel: truly: Add additional delay after pulling down reset gpi=
o
>
> Wang Hai (1):
...
>       drm/amdgpu/discovery: update definitions of table_info and binary_h=
eader
...
>       drm/amd/display: use fixed-width data type for soc bounding box str=
uct
>
> Yannick Fertr=C3=A9 (15):
>       drm/stm: ltdc: disable hw interrupts before its handler init
>       drm/stm: ltdc: fix data enable polarity
>       drm/stm: ltdc: update planes at next vblank to avoid partial refres=
h
...
>
> Yrjan Skrimstad (1):
>       drm/amd/powerplay/smu7_hwmgr: replace blocking delay with non-block=
ing
>
> abdoulaye berthe (1):
...
> is enumulated
>       drm/amdgpu: Use heavy weight for tlb invalidation on xgmi configura=
tion
...
>       drm/amdgpu: Revert "drm/amdgpu: skip gpu reset when ras error occur=
ed"
...
>  create mode 100644
> Documentation/devicetree/bindings/display/allwinner,sun6i-a31-mipi-dsi.ya=
ml
>  create mode 100644 Documentation/devicetree/bindings/display/ingenic,lcd=
.txt
>  create mode 100644
...
>  create mode 100644
> Documentation/devicetree/bindings/display/panel/osddisplays,osd101t2045-5=
3ts.txt
>  create mode 100644
> Documentation/devicetree/bindings/display/panel/osddisplays,osd101t2587-5=
3ts.txt
>  create mode 100644
...
>  rename drivers/gpu/drm/amd/amdgpu/{amdgpu_prime.c =3D> amdgpu_dma_buf.c}=
 (93%)
...
>  create mode 100644 drivers/gpu/drm/amd/amdkfd/cwsr_trap_handler_gfx10.as=
m
>  create mode 100644 drivers/gpu/drm/amd/amdkfd/kfd_device_queue_manager_v=
10.c
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dce100/dce_clk=
_mgr.c
>  create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dce100/dce_clk=
_mgr.h
>  create mode 100644
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn10/rv1_clk_=
mgr.h
>  create mode 100644
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn10/rv2_clk_=
mgr.c
>  rename drivers/gpu/drm/amd/display/dc/{dcn10/dcn10_clk_mgr.h =3D>
> clk_mgr/dcn10/rv2_clk_mgr.h} (82%)
>  create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn20/dcn20_cl=
k_mgr.c
>  create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn20/dcn20_cl=
k_mgr.h
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_link_encod=
er.c
>  create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_link_encod=
er.h
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_stream_enc=
oder.c
>  create mode 100644 drivers/gpu/drm/amd/display/dc/dcn20/dcn20_stream_enc=
oder.h
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/gpio/dcn20/hw_factory_=
dcn20.c
>  create mode 100644 drivers/gpu/drm/amd/display/dc/gpio/dcn20/hw_factory_=
dcn20.h
>  create mode 100644
...
>  create mode 100644 drivers/gpu/drm/amd/display/dc/irq/dcn20/irq_service_=
dcn20.c
>  create mode 100644 drivers/gpu/drm/amd/display/dc/irq/dcn20/irq_service_=
dcn20.h
...
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/clk/clk_11_0_0_o=
ffset.h
>  create mode 100644
> drivers/gpu/drm/amd/include/asic_reg/clk/clk_11_0_0_sh_mask.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/dcn/dcn_2_0_0_of=
fset.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/dcn/dcn_2_0_0_sh=
_mask.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_1_0_def=
ault.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_1_0_off=
set.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_1_0_sh_=
mask.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/hdp/hdp_5_0_0_of=
fset.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/hdp/hdp_5_0_0_sh=
_mask.h
>  create mode 100644
...
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/nbio/nbio_2_3_de=
fault.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/nbio/nbio_2_3_of=
fset.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/nbio/nbio_2_3_sh=
_mask.h
>  create mode 100644
...
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/vcn/vcn_2_0_0_of=
fset.h
>  create mode 100644 drivers/gpu/drm/amd/include/asic_reg/vcn/vcn_2_0_0_sh=
_mask.h
>  create mode 100644 drivers/gpu/drm/amd/include/discovery.h
>  rename drivers/gpu/drm/amd/include/ivsrcid/{ =3D> dcn}/irqsrcs_dcn_1_0.h=
 (100%)
>  create mode 100644 drivers/gpu/drm/amd/include/ivsrcid/gfx/irqsrcs_gfx_1=
0_1.h
>  create mode 100644
...
>  create mode 100644 drivers/gpu/drm/amd/include/ivsrcid/vcn/irqsrcs_vcn_2=
_0.h
...
>  create mode 100644 drivers/gpu/drm/amd/powerplay/inc/smu11_driver_if_nav=
i10.h
...
>  rename drivers/gpu/drm/i915/{ =3D> display}/intel_dp_aux_backlight.c (99=
%)
>  create mode 100644 drivers/gpu/drm/i915/display/intel_dp_aux_backlight.h
>  rename drivers/gpu/drm/i915/{ =3D> display}/intel_dp_link_training.c (99=
%)
...
>  rename drivers/gpu/drm/i915/{ =3D> display}/intel_dsi_dcs_backlight.c (9=
9%)
>  create mode 100644 drivers/gpu/drm/i915/display/intel_dsi_dcs_backlight.=
h
...
>  rename drivers/gpu/drm/i915/{intel_i2c.c =3D> display/intel_gmbus.c} (91=
%)
...
>  rename drivers/gpu/drm/i915/{ =3D> gem}/selftests/huge_gem_object.c (70%=
)
...
>  create mode 100644 drivers/gpu/drm/i915/gem/selftests/i915_gem_client_bl=
t.c
>  rename drivers/gpu/drm/i915/{ =3D> gem}/selftests/i915_gem_coherency.c (=
83%)
>  rename drivers/gpu/drm/i915/{ =3D> gem}/selftests/i915_gem_context.c (80=
%)
>  rename drivers/gpu/drm/i915/{ =3D> gem}/selftests/i915_gem_dmabuf.c (85%=
)
...
>  create mode 100644 drivers/gpu/drm/i915/gem/selftests/i915_gem_object_bl=
t.c
...
>  rename drivers/gpu/drm/i915/{ =3D> gem}/selftests/mock_gem_object.h (65%=
)
...
>  rename drivers/gpu/drm/i915/{selftests/intel_lrc.c =3D> gt/selftest_lrc.=
c} (68%)
