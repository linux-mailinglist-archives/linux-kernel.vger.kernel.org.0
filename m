...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 31D6DBB0FE
	for <lists+linux-kernel@lfdr.de>; Mon, 23 Sep 2019 11:09:03 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1731932AbfIWJId (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 23 Sep 2019 05:08:33 -0400
Received: from mail-eopbgr680113.outbound.protection.outlook.com ([40.107.68.113]:19479
        "EHLO NAM04-BN3-obe.outbound.protection.outlook.com"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1726480AbfIWJIc (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 23 Sep 2019 05:08:32 -0400
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 23 Sep 2019 09:08:16.6039
 (UTC)
...
+
+	if (!mipi_host_node) {
+		prop =3D of_find_property(mipi_host_node, "data-lanes", &len);
+		if (!prop)
+			DRM_ERROR("cannot resolve MIPI host node property.\n");
+		else
+			pdata->dsi_lanes =3D len / sizeof(u32);
+
...
+
+	pdata->node.extcon_node =3D of_graph_get_remote_node(np, 2, 0);
+	pdata->node.internal_panel_node =3D of_graph_get_remote_node(np, 3, 0);
+	pdata->node.edp_node =3D of_graph_get_remote_node(np, 4, 0);
