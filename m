...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 578B3BC452
	for <lists+linux-kernel@lfdr.de>; Tue, 24 Sep 2019 10:57:11 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729297AbfIXI5H (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 24 Sep 2019 04:57:07 -0400
Received: from mail-oi1-f195.google.com ([209.85.167.195]:39578 "EHLO
        mail-oi1-f195.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726311AbfIXI5G (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 24 Sep 2019 04:57:06 -0400
Received: by mail-oi1-f195.google.com with SMTP id w144so954640oia.6
        for <linux-kernel@vger.kernel.org>; Tue, 24 Sep 2019 01:57:02 -0700 (PDT)
...
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        b=
X-Gm-Message-State: APjAAAXY/FbLU2Obabfa5yuEjBnHXKgKumNtTT6Pe0iLxeQpXwzdCVzR
        JxDH2CFxbgyAs6dry/9InSolQGA8+Cw9Sj74WznFfA==
...
References: <1569248002-2485-1-git-send-email-laurentiu.palcu@nxp.com>
 ...
 <CAOcKUNUWrswJ0jZc5SkvYv68wSqL6Js9X0Mey6_w-OmzLyvVRQ@mail.gmail.com> <20190924080355.GC25260@fsr-ub1664-121>
...
On Tue, Sep 24, 2019 at 4:04 PM Laurentiu Palcu <laurentiu.palcu@nxp.com> w=
rote:
>
> Hi,
>
...
ww.nxp.com%2Fwebapp%2FDownload%3FcolCode%3DIMX8MDQLQRM&amp;data=3D02%7C01%7=
Claurentiu.palcu%40nxp.com%7C94b752dadd2e413c70d008d740a32979%7C686ea1d3bc2=
b4c6fa92cd99c5c301635%7C0%7C0%7C637048941849367464&amp;sdata=3DrsFNo5MGwdFG=
TAheU8YrJeRLvThpwbGG9bC9hKxCsm0%3D&amp;reserved=3D0
> > >
> > > The current patch adds only basic functionality: one primary plane fo=
r
...
The different ways of accessing register can be wrapped by some
helpers, so it doesn't seem to bring much code for platform sense.
...
Is it complicated enough to break the idea of one DPR driver solution?
...
Understand.  But, it's worth evaluating how complicated it can be.
...
> > >
> > > diff --git a/drivers/gpu/drm/imx/Kconfig b/drivers/gpu/drm/imx/Kconfi=
g
...
> > > +         Choose this if you have a NXP i.MX8MQ based system and want=
 to use the
> > > +         Display Controller Subsystem. This option enables DCSS supp=
ort.
...
dcss-dtg.o \
> > > +                                dcss-ss.o dcss-dpr.o dcss-scaler.o d=
css-kms.o dcss-crtc.o \
> > > +                                dcss-plane.o
> > > +
...
> > > +               dcss_writel(0, blkctl->base_reg + DCSS_BLKCTL_CONTROL=
0);
> > > +       else
> > > +               dcss_writel(DISPMIX_PIXCLK_SEL,
> > > +                           blkctl->base_reg + DCSS_BLKCTL_CONTROL0);
> > > +
> > > +       dcss_set(B_CLK_RESETN | APB_CLK_RESETN | P_CLK_RESETN | RTR_C=
LK_RESETN,
...
> > > +
> > > +       blkctl =3D devm_kzalloc(dcss->dev, sizeof(*blkctl), GFP_KERNE=
L);
> > > +       if (!blkctl)
> > > +               return -ENOMEM;
> > > +
> > > +       blkctl->base_reg =3D devm_ioremap(dcss->dev, blkctl_base, SZ_=
4K);
> > > +       if (!blkctl->base_reg) {
...
> > > +       blkctl->dev =3D dcss->dev;
> > > +       blkctl->hdmi_output =3D dcss->hdmi_output;
> > > +
...
> > > +{
> > > +       struct dcss_crtc *dcss_crtc =3D container_of(crtc, struct dcs=
s_crtc,
> > > +                                                  base);
...
> > > +
> > > +       dcss_crtc->irq_enabled =3D false;
> > > +}
> > > +
...
> > > +                                  struct drm_crtc_state *old_crtc_st=
ate)
> > > +{
...
> > > +       dcss_ss_sync_set(dcss->ss, &vm, mode->flags & DRM_MODE_FLAG_P=
HSYNC,
...
> > > +                                    struct drm_crtc_state *old_crtc_=
state)
> > > +{
> > > +       struct dcss_crtc *dcss_crtc =3D container_of(crtc, struct dcs=
s_crtc,
...
> > > +       struct dcss_dev *dcss =3D dcss_crtc->base.dev->dev_private;
> > > +
> > > +       drm_atomic_helper_disable_planes_on_crtc(old_crtc_state, fals=
e);
> > > +
> > > +       spin_lock_irq(&crtc->dev->event_lock);
> > > +       if (crtc->state->event) {
...
> > > +
> > > +       ret =3D devm_request_irq(dcss->dev, crtc->irq, dcss_crtc_irq_=
handler,
...
> > > +#define CTXLD_IRQ_COMPLETION           (DB_COMP | SB_HP_COMP | SB_LP=
_COMP)
> > > +#define CTXLD_IRQ_ERROR                        (RD_ERR | DB_PEND_SB_=
REC | AHB_ERR)
> > > +
...
> > > +
> > > +       u16 ctx_size[2][3]; /* holds the sizes of DB, SB_HP and SB_LP=
 ctx */
> > > +       u8 current_ctx;
> > > +
> > > +       bool in_use;
...
> > > +       u32 irq_status;
> > > +
> > > +       irq_status =3D dcss_readl(ctxld->ctxld_reg + DCSS_CTXLD_CONTR=
OL_STATUS);
> > > +
...
> > > +                       ctxld->dtg_disable_cb =3D NULL;
> > > +                       ctxld->dtg_disable_data =3D NULL;
> > > +               }
> > > +       } else if (irq_status & CTXLD_IRQ_ERROR) {
> > > +               /*
> > > +                * Except for throwing an error message and clearing =
the status
...
",
> > > +                       irq_status);
> > > +               dev_err(ctxld->dev, "ctxld: db=3D%d, sb_hp=3D%d, sb_l=
p=3D%d\n",
> > > +                       ctxld->ctx_size[ctxld->current_ctx ^ 1][CTX_D=
B],
> > > +                       ctxld->ctx_size[ctxld->current_ctx ^ 1][CTX_S=
B_HP],
> > > +                       ctxld->ctx_size[ctxld->current_ctx ^ 1][CTX_S=
B_LP]);
> > > +       }
...
> > > +       int ret;
> > > +
> > > +       ctxld->irq =3D platform_get_irq_byname(pdev, CTXLD_IRQ_NAME);
> > > +       if (ctxld->irq < 0) {
> > > +               dev_err(ctxld->dev, "ctxld: can't get irq number\n");
> > > +               return ctxld->irq;
> > > +       }
> > > +
> > > +       ret =3D devm_request_irq(ctxld->dev, ctxld->irq,
...
> > > +                                          CTXLD_DB_CTX_ENTRIES * siz=
eof(*ctx),
> > > +                                          ctxld->db[i], ctxld->db_pa=
ddr[i]);
> > > +                       ctxld->db[i] =3D NULL;
...
> > > +               if (ctxld->sb_hp[i]) {
> > > +                       dmam_free_coherent(ctxld->dev,
> > > +                                          CTXLD_SB_CTX_ENTRIES * siz=
eof(*ctx),
> > > +                                          ctxld->sb_hp[i], ctxld->sb=
_paddr[i]);
...
> > > +
> > > +       for (i =3D 0; i < 2; i++) {
> > > +               ctx =3D dmam_alloc_coherent(ctxld->dev,
> > > +                                         CTXLD_DB_CTX_ENTRIES * size=
of(*ctx),
...
> > > +
> > > +               ctxld->db[i] =3D ctx;
> > > +               ctxld->db_paddr[i] =3D dma_handle;
> > > +
> > > +               ctx =3D dmam_alloc_coherent(ctxld->dev,
> > > +                                         CTXLD_SB_CTX_ENTRIES * size=
of(*ctx),
> > > +                                         &dma_handle, GFP_KERNEL);
> > > +               if (!ctx)
> > > +                       return -ENOMEM;
...
> > > +
> > > +               ctxld->sb_paddr[i] =3D dma_handle;
> > > +       }
> > > +
> > > +       return 0;
> > > +}
...
> > > +       int ret;
> > > +
> > > +       ctxld =3D devm_kzalloc(dcss->dev, sizeof(struct dcss_ctxld),
> > > +                            GFP_KERNEL);
> > > +       if (!ctxld)
> > > +               return -ENOMEM;
> > > +
...
> > > +       if (ret) {
> > > +               dev_err(dcss->dev, "ctxld: cannot allocate context me=
mory.\n");
> > > +               goto err;
> > > +       }
> > > +
> > > +       ctxld->ctxld_reg =3D devm_ioremap(dcss->dev, ctxld_base, SZ_4=
K);
> > > +       if (!ctxld->ctxld_reg) {
...
> > > +       }
> > > +
> > > +       ret =3D dcss_ctxld_irq_config(ctxld, to_platform_device(dcss-=
>dev));
> > > +       if (ret)
> > > +               goto err_irq;
> > > +
...
> > > +       if (sb_lp_cnt &&
> > > +           ctxld->sb_lp[curr_ctx] !=3D ctxld->sb_hp[curr_ctx] + sb_h=
p_cnt) {
...
> > > +
> > > +       dcss_writel(db_base, ctxld->ctxld_reg + DCSS_CTXLD_DB_BASE_AD=
DR);
> > > +       dcss_writel(db_cnt, ctxld->ctxld_reg + DCSS_CTXLD_DB_COUNT);
> > > +
> > > +       if (sb_hp_cnt)
> > > +               sb_count =3D ((sb_hp_cnt << SB_HP_COUNT_POS) & SB_HP_=
COUNT_MASK) |
> > > +                          ((sb_lp_cnt << SB_LP_COUNT_POS) & SB_LP_CO=
UNT_MASK);
> > > +       else
> > > +               sb_count =3D (sb_lp_cnt << SB_HP_COUNT_POS) & SB_HP_C=
OUNT_MASK;
> > > +
> > > +       sb_base =3D sb_count ? ctxld->sb_paddr[curr_ctx] : 0;
> > > +
> > > +       dcss_writel(sb_base, ctxld->ctxld_reg + DCSS_CTXLD_SB_BASE_AD=
DR);
> > > +       dcss_writel(sb_count, ctxld->ctxld_reg + DCSS_CTXLD_SB_COUNT)=
;
> > > +
> > > +       /* enable the context loader */
> > > +       dcss_set(CTXLD_ENABLE, ctxld->ctxld_reg + DCSS_CTXLD_CONTROL_=
STATUS);
> > > +
> > > +       ctxld->in_use =3D true;
> > > +
> > > +       /*
> > > +        * Toggle the current context to the alternate one so that an=
y updates
...
&
> > > +               ctxld->ctx_size[ctxld->current_ctx][CTX_SB_HP] =3D=3D=
 0 &&
> > > +               ctxld->ctx_size[ctxld->current_ctx][CTX_SB_LP] =3D=3D=
 0;
> > > +}
> > > +
...
> > > +                                       void *data)
> > > +{
> > > +       ctxld->dtg_disable_cb =3D cb;
> > > +       ctxld->dtg_disable_data =3D data;
> > > +}
...
> > > +
> > > +       ret =3D dcss_blkctl_init(dcss, base_addr + devtype->blkctl_of=
s);
> > > +       if (ret)
> > > +               return ret;
> > > +
...
> > > +               goto dpr_err;
> > > +
> > > +       ret =3D dcss_scaler_init(dcss, base_addr + devtype->scaler_of=
s);
> > > +       if (ret)
> > > +               goto scaler_err;
> > > +
...
> > > +       }
> > > +
> > > +       dcss =3D devm_kzalloc(dev, sizeof(struct dcss_dev), GFP_KERNE=
L);
> > > +       if (!dcss)
...
> > > +                        const struct drm_format_info *format, u64 mo=
difier);
...
> > > +                             const struct drm_format_info *format, i=
nt alpha);
...
> > > +                      int src_xres, int src_yres, int dst_xres, int =
dst_yres,
> > > +                      u32 vrefresh_hz);
...
> > > +
> > > +       dcss_ctxld_write(dpr->ctxld, dpr->ctx_id, val, ch->base_ofs +=
 ofs);
> > > +}
> > > +
...
> > and PRG(s) can be reflected in device tree.  So, it would be good if th=
e
...
> >
> > > +
> > > +               ch->base_reg =3D devm_ioremap(dpr->dev, ch->base_ofs,=
 SZ_4K);
> > > +               if (!ch->base_reg) {
> > > +                       dev_err(dpr->dev, "dpr: unable to remap ch %d=
 base\n",
> > > +                               i);
> > > +                       return -ENOMEM;
> > > +               }
...
> > > +       struct dcss_dpr *dpr;
> > > +
> > > +       dpr =3D devm_kzalloc(dcss->dev, sizeof(struct dcss_dpr), GFP_=
KERNEL);
> > > +       if (!dpr)
> > > +               return -ENOMEM;
> > > +
...
> > > +                               devm_iounmap(dpr->dev, dpr->ch[i].bas=
e_reg);
> > > +               }
> > > +
...
> > > +
> > > +       if (pix_format =3D=3D DRM_FORMAT_P010)
> > > +               pix_wide =3D pix_wide * 10 / 8;
> > > +
> > > +       div_64byte_mod =3D pix_wide % pix_in_64byte;
> > > +       offset =3D (div_64byte_mod =3D=3D 0) ? 0 : (pix_in_64byte - d=
iv_64byte_mod);
> > > +
...
> > > +       u32 gap =3D DCSS_DPR_FRAME_2P_BASE_ADDR - DCSS_DPR_FRAME_1P_B=
ASE_ADDR;
...
> > > +
> > > +               pix_x_wide =3D dcss_dpr_x_pix_wide_adjust(ch, xres, p=
ix_format);
> > > +               pix_y_high =3D dcss_dpr_y_pix_high_adjust(ch, yres, p=
ix_format);
> > > +
> > > +               if (plane =3D=3D 0)
> > > +                       ch->pitch =3D pix_x_wide;
> > > +
> > > +               dcss_dpr_write(ch, pix_x_wide,
> > > +                              DCSS_DPR_FRAME_1P_PIX_X_CTRL + plane *=
 gap);
> > > +               dcss_dpr_write(ch, pix_y_high,
> > > +                              DCSS_DPR_FRAME_1P_PIX_Y_CTRL + plane *=
 gap);
> > > +
> > > +               dcss_dpr_write(ch, 2, DCSS_DPR_FRAME_1P_CTRL0 + plane=
 * gap);
> > > +       }
> > > +}
> > > +
...
> > > +       struct dcss_dpr_ch *ch =3D &dpr->ch[ch_num];
> > > +
> > > +       dcss_dpr_write(ch, luma_base_addr, DCSS_DPR_FRAME_1P_BASE_ADD=
R);
> > > +
> > > +       dcss_dpr_write(ch, chroma_base_addr, DCSS_DPR_FRAME_2P_BASE_A=
DDR);
> > > +
...
> > > +                                 const struct drm_format_info *forma=
t)
> > > +{
> > > +       u32 val;
> > > +
...
> > > +               dcss_dpr_write(ch, ch->frame_ctrl, DCSS_DPR_FRAME_CTR=
L0);
> > > +               dcss_dpr_write(ch, ch->rtram_ctrl, DCSS_DPR_RTRAM_CTR=
L0);
> > > +       }
> > > +
...
> > > +                                &g_sel, &b_sel)) {
> > > +                       dcss_dpr_argb_comp_sel(ch, a_sel, r_sel, g_se=
l, b_sel);
> > > +               } else {
...
> > > +       ch->mode_ctrl |=3D ((ch->tile << TILE_TYPE_POS) & TILE_TYPE_M=
ASK);
> > > +}
> > > +
> > > +void dcss_dpr_format_set(struct dcss_dpr *dpr, int ch_num,
...
> > > +
> > > +               if (ch->sys_ctrl_chgd) {
> > > +                       dcss_ctxld_write_irqsafe(dpr->ctxld, dpr->ctx=
_id,
> > > +                                                ch->sys_ctrl,
> > > +                                                ch->base_ofs +
> > > +                                                DCSS_DPR_SYSTEM_CTRL=
0);
...
> > > +
> > > +       ch->frame_ctrl |=3D rotation & DRM_MODE_REFLECT_X ? HFLIP_EN =
: 0;
> > > +       ch->frame_ctrl |=3D rotation & DRM_MODE_REFLECT_Y ? VFLIP_EN =
: 0;
> > > +
...
> > > +       of_node_put(remote);
> > > +
> > > +       return component_master_add_with_match(dev, &dcss_master_ops,=
 match);
> > > +}
> > > +
...
> > > +       { .compatible =3D "nxp,imx8mq-dcss", .data =3D &dcss_types[DC=
SS_IMX8MQ], },
> > > +       {},
> > > +};
> > > +
...
> > > +#define   CH1_ALPHA_SEL                                        BIT(1=
0)
> > > +#define   CSS_PIX_COMP_SWAP_POS                                12
> > > +#define   CSS_PIX_COMP_SWAP_MASK                       GENMASK(14, 1=
2)
> > > +#define   DEFAULT_FG_ALPHA_POS                         24
> > > +#define   DEFAULT_FG_ALPHA_MASK                                GENMA=
SK(31, 24)
...
> > > +
> > > +       /*
> > > +        * This will be passed on by DRM CRTC so that we can signal w=
hen DTG has
> > > +        * been successfully stopped. Otherwise, any modesetting whil=
e DTG is
...
> > > +
> > > +       dcss_ctxld_write(dtg->ctxld, dtg->ctx_id, val, dtg->base_ofs =
+ ofs);
> > > +}
> > > +
...
> > > +
> > > +       dcss_writel(status & LINE0_IRQ, dtg->base_reg + DCSS_DTG_INT_=
CONTROL);
> > > +
> > > +       return IRQ_HANDLED;
> > > +}
...
> > > +       int ret;
> > > +
> > > +       dtg->ctxld_kick_irq =3D platform_get_irq_byname(pdev, "ctxld_=
kick");
...
> > > +       }
> > > +
> > > +       ret =3D devm_request_irq(dtg->dev, dtg->ctxld_kick_irq,
...
> > > +
> > > +       dcss_update(LINE0_IRQ, LINE0_IRQ, dtg->base_reg + DCSS_DTG_IN=
T_MASK);
> > > +
> > > +       return 0;
> > > +}
...
> > > +       struct dcss_dtg *dtg;
> > > +
> > > +       dtg =3D devm_kzalloc(dcss->dev, sizeof(*dtg), GFP_KERNEL);
> > > +       if (!dtg)
> > > +               return -ENOMEM;
> > > +
> > > +       dcss->dtg =3D dtg;
...
> > > +
> > > +       dtg->base_reg =3D devm_ioremap(dcss->dev, dtg_base, SZ_4K);
> > > +       if (!dtg->base_reg) {
...
> > > +
> > > +       dtg->pix_clk =3D dcss->pix_clk;
> > > +       dtg->pll_src_clk =3D dcss->pll_src_clk;
> > > +       dtg->pll_phy_ref_clk =3D dcss->pll_phy_ref_clk;
> > > +
> > > +       dtg->alpha =3D 255;
> > > +
> > > +       dtg->control_status |=3D OVL_DATA_MODE | BLENDER_VIDEO_ALPHA_=
SEL |
> > > +               ((dtg->alpha << DEFAULT_FG_ALPHA_POS) & DEFAULT_FG_AL=
PHA_MASK);
> > > +
...
> > > +{
> > > +       /* stop DTG */
> > > +       dcss_writel(DTG_START, dtg->base_reg + DCSS_DTG_TC_CONTROL_ST=
ATUS);
> > > +
...
> > > +       u32 actual_clk;
> > > +
> > > +       dtg_lrc_x =3D vm->hfront_porch + vm->hback_porch + vm->hsync_=
len +
> > > +                   vm->hactive - 1;
> > > +       dtg_lrc_y =3D vm->vfront_porch + vm->vback_porch + vm->vsync_=
len +
> > > +                   vm->vactive - 1;
> > > +       dis_ulc_x =3D vm->hsync_len + vm->hback_porch - 1;
> > > +       dis_ulc_y =3D vm->vsync_len + vm->vfront_porch + vm->vback_po=
rch - 1;
> > > +       dis_lrc_x =3D vm->hsync_len + vm->hback_porch + vm->hactive -=
 1;
> > > +       dis_lrc_y =3D vm->vsync_len + vm->vfront_porch + vm->vback_po=
rch +
> > > +                   vm->vactive - 1;
> > > +
...
> > > +
> > > +       actual_clk =3D clk_get_rate(dtg->pix_clk);
...
> > > +                                                       TC_CTXLD_SB_Y=
_MASK;
> > > +       db_ctxld_trig =3D ((99 * dis_lrc_y / 100) << TC_CTXLD_DB_Y_PO=
S) &
> > > +                                                       TC_CTXLD_DB_Y=
_MASK;
> > > +
> > > +       dcss_dtg_write(dtg, sb_ctxld_trig | db_ctxld_trig, DCSS_DTG_T=
C_CTXLD);
> > > +
> > > +       /* vblank trigger */
...
> > > +
> > > +       /* CTXLD trigger */
> > > +       dcss_dtg_write(dtg, ((90 * dis_lrc_y) / 100) << 16, DCSS_DTG_=
LINE0_INT);
> > > +}
> > > +
> > > +void dcss_dtg_plane_pos_set(struct dcss_dtg *dtg, int ch_num,
...
> > > +
> > > +       if (!px && !py && !pw && !ph) {
> > > +               dcss_dtg_write(dtg, 0, DCSS_DTG_TC_CH1_TOP + 0x8 * ch=
_num);
> > > +               dcss_dtg_write(dtg, 0, DCSS_DTG_TC_CH1_BOT + 0x8 * ch=
_num);
> > > +       } else {
...
> > > +               return;
> > > +
> > > +       alpha_val =3D (alpha << DEFAULT_FG_ALPHA_POS) & DEFAULT_FG_AL=
PHA_MASK;
> > > +
> > > +       /*
> > > +        * Use global alpha if pixel format does not have alpha chann=
el or the
> > > +        * user explicitly chose to use global alpha (i.e. alpha is n=
ot OPAQUE).
> > > +        */
> > > +       if (!format->has_alpha || alpha !=3D 255) {
> > > +               dtg->control_status &=3D ~(CH1_ALPHA_SEL | DEFAULT_FG=
_ALPHA_MASK);
...
> > > +                       (0x5 << CSS_PIX_COMP_SWAP_POS) & CSS_PIX_COMP=
_SWAP_MASK;
> > > +}
> > > +
...
> > > +{
> > > +       if (!en) {
> > > +               dtg->dis_completion =3D dis_completion;
...
> > > +       }
> > > +
> > > +       dtg->dis_completion =3D NULL;
> > > +
> > > +       dtg->control_status |=3D DTG_START;
> > > +
> > > +       dcss_dtg_write(dtg, dtg->control_status, DCSS_DTG_TC_CONTROL_=
STATUS);
> > > +
> > > +       dtg->in_use =3D true;
> > > +}
...
> > > +               dcss_dtg_write(dtg, control_status, DCSS_DTG_TC_CONTR=
OL_STATUS);
> > > +
...
> > > +       u32 status;
> > > +
> > > +       dcss_update(LINE1_IRQ, LINE1_IRQ, dtg->base_reg + DCSS_DTG_IN=
T_MASK);
> > > +
> > > +       if (en) {
> > > +               status =3D dcss_readl(dtg->base_reg + DCSS_DTG_INT_ST=
ATUS);
...
> > > +                                   dtg->base_reg + DCSS_DTG_INT_CONT=
ROL);
...
> > > +bool dcss_dtg_vblank_irq_valid(struct dcss_dtg *dtg)
> > > +{
> > > +       return !!(dcss_readl(dtg->base_reg + DCSS_DTG_INT_STATUS) & L=
INE1_IRQ);
> > > +}
> > > +
...
> > > +};
> > > +
> > > +static void dcss_drm_atomic_commit_tail(struct dcss_drm_commit *comm=
it)
> > > +{
> > > +       struct drm_atomic_state *state =3D commit->state;
> > > +       struct drm_device *drm =3D commit->drm;
> > > +       struct dcss_kms_dev *kms =3D container_of(drm, struct dcss_km=
s_dev, base);
> > > +
...
> > > +{
> > > +       struct dcss_drm_commit *commit =3D container_of(work,
> > > +                                                     struct dcss_drm=
_commit,
> > > +                                                     work);
> > > +
...
> > > +               ret =3D drm_atomic_helper_prepare_planes(drm, state);
> > > +               if (ret)
> > > +                       return ret;
> > > +
...
> > > +
> > > +       if (!nonblock) {
> > > +               ret =3D drm_atomic_helper_wait_for_fences(drm, state,=
 true);
> > > +               if (ret)
> > > +                       goto err;
> > > +       }
> > > +
> > > +       spin_lock(&kms->commit.wait.lock);
> > > +       ret =3D wait_event_interruptible_locked(kms->commit.wait,
> > > +                                             !kms->commit.pending);
> > > +       if (ret =3D=3D 0)
> > > +               kms->commit.pending =3D true;
> > > +       spin_unlock(&kms->commit.wait.lock);
> > > +
> > > +       if (ret)
> > > +               goto err;
> > > +
> > > +       ret =3D drm_atomic_helper_swap_state(state, true);
> > > +       if (ret)
> > > +               goto err;
> > > +
...
> > > +}
> > > +
> > > +const struct drm_mode_config_funcs dcss_drm_mode_config_funcs =3D {
> > > +       .fb_create =3D drm_gem_fb_create,
> > > +       .output_poll_changed =3D drm_fb_helper_output_poll_changed,
> > > +       .atomic_check =3D drm_atomic_helper_check,
> > > +       .atomic_commit =3D dcss_drm_atomic_commit,
> > > +};
> > > +
> > > +static struct drm_driver dcss_kms_driver =3D {
> > > +       .driver_features        =3D DRIVER_MODESET | DRIVER_GEM | DRI=
VER_ATOMIC,
...
> > > +               goto cleanup_mode_config;
> > > +
> > > +       kms->commit_wq =3D alloc_ordered_workqueue("dcss_nonblock_com=
mit_wq", 0);
> > > +       if (!kms->commit_wq) {
> > > +               ret =3D -ENOMEM;
> > > +               goto cleanup_crtc;
...
> > > +{
> > > +       struct dcss_plane *dcss_plane =3D container_of(plane, struct =
dcss_plane,
> > > +                                                    base);
> > > +
...
||
> > > +                              modifier =3D=3D DRM_FORMAT_MOD_VIVANTE=
_TILED ||
> > > +                              modifier =3D=3D DRM_FORMAT_MOD_VIVANTE=
_SUPER_TILED;
> > > +               default:
...
> > > +                            (mod_present && modifier =3D=3D DRM_FORM=
AT_MOD_LINEAR);
> > > +       u32 supported_rotation =3D DRM_MODE_ROTATE_0;
> > > +
> > > +       if (!format->is_yuv && linear_format)
> > > +               supported_rotation =3D DRM_MODE_ROTATE_0 | DRM_MODE_R=
OTATE_180 |
...
> > > +{
> > > +       if (src_w < 64 &&
> > > +           (pix_fmt =3D=3D DRM_FORMAT_NV12 || pix_fmt =3D=3D DRM_FOR=
MAT_NV21))
> > > +               return false;
> > > +       else if (src_w < 32 &&
> > > +                (pix_fmt =3D=3D DRM_FORMAT_UYVY || pix_fmt =3D=3D DR=
M_FORMAT_VYUY ||
> > > +                 pix_fmt =3D=3D DRM_FORMAT_YUYV || pix_fmt =3D=3D DR=
M_FORMAT_YVYU))
> > > +               return false;
> > > +
...
> > > +       bool is_primary_plane =3D plane->type =3D=3D DRM_PLANE_TYPE_P=
RIMARY;
...
> > > +       WARN_ON(!cma_obj);
> > > +
> > > +       crtc_state =3D drm_atomic_get_existing_crtc_state(state->stat=
e,
> > > +                                                       state->crtc);
> > > +
...
> > > +       }
> > > +
> > > +       dcss_scaler_get_min_max_ratios(dcss->scaler, dcss_plane->ch_n=
um,
> > > +                                      &min, &max);
> > > +
...
> > > +               DRM_DEBUG_KMS("requested cropping operation is not al=
lowed!\n");
> > > +               return -EINVAL;
> > > +       }
> > > +
...
> > > +       struct drm_gem_cma_object *cma_obj =3D drm_fb_cma_get_gem_obj=
(fb, 0);
...
> > > +                       format->char_per_block[0] * (state->src.x1 >>=
 16);
...
> > > +
> > > +       if (old_state->fb && !drm_atomic_crtc_needs_modeset(crtc_stat=
e) &&
...
> > > +           modifiers_present && fb->modifier =3D=3D DRM_FORMAT_MOD_L=
INEAR)
> > > +               modifiers_present =3D false;
> > > +
> > > +       dcss_dpr_format_set(dcss->dpr, dcss_plane->ch_num, state->fb-=
>format,
...
> > > +       dcss_scaler_ch_enable(dcss->scaler, dcss_plane->ch_num, enabl=
e);
> > > +
> > > +       if (!enable)
> > > +               dcss_dtg_plane_pos_set(dcss->dtg, dcss_plane->ch_num,
...
> > > +                                     struct drm_plane_state *old_sta=
te)
> > > +{
> > > +       struct dcss_plane *dcss_plane =3D to_dcss_plane(plane);
> > > +       struct dcss_dev *dcss =3D plane->dev->dev_private;
...
> > > +       dcss_dtg_plane_pos_set(dcss->dtg, dcss_plane->ch_num, 0, 0, 0=
, 0);
...
> > > +
> > > +       ret =3D drm_universal_plane_init(drm, &dcss_plane->base, poss=
ible_crtcs,
> > > +                                      &dcss_plane_funcs, dcss_common=
_formats,
...
> > > +       }
> > > +
> > > +       drm_plane_helper_add(&dcss_plane->base, &dcss_plane_helper_fu=
ncs);
> > > +
> > > +       ret =3D drm_plane_create_zpos_immutable_property(&dcss_plane-=
>base, zpos);
> > > +       if (ret)
> > > +               return ERR_PTR(ret);
> > > +
...
> > > +#define   OFIFO_HIGH_THRES_MASK                        GENMASK(25, 1=
6)
...
> > > +       coef[phase_cnt & PSC_PHASE_MASK][tap_cnt1 >> PSC_BITS_FOR_PHA=
SE] =3D g0_q;
> > > +
> > > +       for (i =3D 0; i < mid; i++) {
> > > +               phase_cnt++;
...
> > > +                       coef[phase_cnt & PSC_PHASE_MASK][tap_idx] =3D=
 g0_q;
> > > +               }
> > > +               if (((-phase_cnt) & PSC_PHASE_MASK) <=3D 8) {
> > > +                       tap_idx =3D tap_cnt2 >> PSC_BITS_FOR_PHASE;
> > > +                       coef[(-phase_cnt) & PSC_PHASE_MASK][tap_idx] =
=3D g0_q;
> > > +               }
> > > +       }
> > > +
...
,
> > > +                                     bool use_5_taps, bool phase0_id=
entity,
> > > +                                     int coef[][PSC_NUM_TAPS])
> > > +{
> > > +       int fc_q;
...
> > > +       else
> > > +               fc_q =3D div_q(dst_length, src_length * PSC_NUM_PHASE=
S);
> > > +
> > > +       /* compute gaussian filter coefficients */
> > > +       dcss_scaler_gaussian_filter(fc_q, use_5_taps, phase0_identity=
, coef);
> > > +}
> > > +
...
> > > +
> > > +       dcss_ctxld_write(scl->ctxld, scl->ctx_id, val, ch->base_ofs +=
 ofs);
> > > +}
> > > +
...
> > > +
> > > +               ch->base_reg =3D devm_ioremap(scl->dev, ch->base_ofs,=
 SZ_4K);
> > > +               if (!ch->base_reg) {
> > > +                       dev_err(scl->dev, "scaler: unable to remap ch=
 base\n");
> > > +                       return -ENOMEM;
> > > +               }
> > > +
...
> > > +       struct dcss_scaler *scaler;
> > > +
> > > +       scaler =3D devm_kzalloc(dcss->dev, sizeof(*scaler), GFP_KERNE=
L);
> > > +       if (!scaler)
> > > +               return -ENOMEM;
> > > +
...
> > > +
> > > +       if (en)
> > > +               dcss_scaler_write(ch, ch->sdata_ctrl, DCSS_SCALER_SDA=
TA_CTRL);
> > > +
...
> > > +                         ((val << CHR_BIT_DEPTH_POS) & CHR_BIT_DEPTH=
_MASK) |
> > > +                         ((val << LUM_BIT_DEPTH_POS) & LUM_BIT_DEPTH=
_MASK),
...
> > > +                               u32 pix_format, enum buffer_format ds=
t_format)
> > > +{
...
> > > +
> > > +       if (pix_format =3D=3D DRM_FORMAT_UYVY || pix_format =3D=3D DR=
M_FORMAT_VYUY ||
> > > +           pix_format =3D=3D DRM_FORMAT_YUYV || pix_format =3D=3D DR=
M_FORMAT_YVYU) {
> > > +               csrc_xres >>=3D 1;
> > > +               src_is_444 =3D false;
...
> > > +               cdst_xres >>=3D 1;
> > > +
> > > +       /* for 4:4:4 to 4:2:2 conversion, source height should be 1 l=
ess */
...
> > > +       }
> > > +
> > > +       dcss_scaler_write(ch, (((lsrc_yres - 1) << HEIGHT_POS) & HEIG=
HT_MASK) |
> > > +                              (((lsrc_xres - 1) << WIDTH_POS) & WIDT=
H_MASK),
> > > +                         DCSS_SCALER_SRC_LUM_RES);
> > > +       dcss_scaler_write(ch, (((csrc_yres - 1) << HEIGHT_POS) & HEIG=
HT_MASK) |
> > > +                              (((csrc_xres - 1) << WIDTH_POS) & WIDT=
H_MASK),
> > > +                         DCSS_SCALER_SRC_CHR_RES);
> > > +       dcss_scaler_write(ch, (((ldst_yres - 1) << HEIGHT_POS) & HEIG=
HT_MASK) |
> > > +                              (((ldst_xres - 1) << WIDTH_POS) & WIDT=
H_MASK),
> > > +                         DCSS_SCALER_DST_LUM_RES);
> > > +       dcss_scaler_write(ch, (((cdst_yres - 1) << HEIGHT_POS) & HEIG=
HT_MASK) |
> > > +                              (((cdst_xres - 1) << WIDTH_POS) & WIDT=
H_MASK),
...
> > > +};
> > > +
> > > +static const struct dcss_scaler_ratios dcss_scaler_ratios[] =3D {
...
> > > +                                     enum chroma_location src_chroma=
_loc)
> > > +{
...
> > > +                        */
> > > +                       c_vstart -=3D (1 << (PSC_PHASE_FRACTION_BITS =
- 2));
> > > +                       break;
...
> > > +               case PSC_LOC_HORZ_1_OVER_4_VERT_1_OVER_2:
> > > +                       /* move chroma left 1/4 chroma input sample s=
pacing */
> > > +                       c_hstart -=3D (1 << (PSC_PHASE_FRACTION_BITS =
- 2));
> > > +                       break;
> > > +               default:
> > > +                       break;
...
> > > +       c_vinc =3D ((src_c_yres << 13) + (dst_c_yres >> 1)) / dst_c_y=
res;
> > > +       l_hinc =3D ((src_xres << 13) + (dst_xres >> 1)) / dst_xres;
> > > +       c_hinc =3D ((src_c_xres << 13) + (dst_c_xres >> 1)) / dst_c_x=
res;
> > > +
...
> > > +                                  int *min, int *max)
> > > +{
> > > +       *min =3D dcss_scaler_ratios[ch_num].upscale;
> > > +       *max =3D dcss_scaler_ratios[ch_num].downscale;
> > > +
> > > +       return 0;
> > > +}
> > > +
...
> > > +                              src_format =3D=3D BUF_FMT_ARGB8888_YUV=
444);
> > > +
> > > +       /* horizontal luma */
> > > +       dcss_scaler_filter_design(src_xres, dst_xres, false,
> > > +                                 src_xres =3D=3D dst_xres, coef);
> > > +       dcss_scaler_program_7_coef_set(ch, DCSS_SCALER_COEF_HLUM, coe=
f);
> > > +
> > > +       /* vertical luma */
...
> > > +
> > > +       if (program_5_taps)
> > > +               dcss_scaler_program_5_coef_set(ch, DCSS_SCALER_COEF_V=
LUM, coef);
> > > +       else
> > > +               dcss_scaler_program_7_coef_set(ch, DCSS_SCALER_COEF_V=
LUM, coef);
> > > +
...
> > > +       /* horizontal chroma */
> > > +       dcss_scaler_filter_design(src_xres, dst_xres, false,
> > > +                                 (src_xres =3D=3D dst_xres) && (ch->=
c_hstart =3D=3D 0),
> > > +                                 coef);
> > > +
> > > +       dcss_scaler_program_7_coef_set(ch, DCSS_SCALER_COEF_HCHR, coe=
f);
> > > +
> > > +       /* vertical chroma */
> > > +       dcss_scaler_filter_design(src_yres, dst_yres, program_5_taps,
> > > +                                 (src_yres =3D=3D dst_yres) && (ch->=
c_vstart =3D=3D 0),
> > > +                                 coef);
> > > +       if (program_5_taps)
> > > +               dcss_scaler_program_5_coef_set(ch, DCSS_SCALER_COEF_V=
...
> > > +                                 src_yres =3D=3D dst_yres, coef);
> > > +       dcss_scaler_program_7_coef_set(ch, DCSS_SCALER_COEF_VLUM, coe=
f);
> > > +}
> > > +
...
> > > +       }
> > > +
> > > +       ch->sdata_ctrl |=3D a2r10g10b10_format << A2R10G10B10_FORMAT_=
POS;
> > > +}
> > > +
> > > +void dcss_scaler_setup(struct dcss_scaler *scl, int ch_num,
...
> > > +               if (pix_format =3D=3D DRM_FORMAT_NV12 ||
> > > +                   pix_format =3D=3D DRM_FORMAT_NV21 ||
> > > +                   pix_format =3D=3D DRM_FORMAT_P010) {
...
> > > +       dcss_scaler_res_set(ch, src_xres, src_yres, dst_xres, dst_yre=
s,
...
> > > +                       dcss_ctxld_write_irqsafe(scl->ctxld, scl->ctx=
_id,
> > > +                                                ch->scaler_ctrl,
> > > +                                                ch->base_ofs +
> > > +                                                DCSS_SCALER_CTRL);
...
> > > +#define   SYNC_END_MASK                                GENMASK(28, 1=
6)
...
> > > +#define   CLIP_MAX_MASK                                GENMASK(23, 1=
6)
...
> > > +       struct dcss_ss *ss;
> > > +
> > > +       ss =3D devm_kzalloc(dcss->dev, sizeof(*ss), GFP_KERNEL);
> > > +       if (!ss)
> > > +               return -ENOMEM;
> > > +
> > > +       dcss->ss =3D ss;
...
> > > +
> > > +       ss->base_reg =3D devm_ioremap(dcss->dev, ss_base, SZ_4K);
> > > +       if (!ss->base_reg) {
...
> > > +               vm->vactive - 1;
> > > +
> > > +       dcss_ss_write(ss, (lrc_y << LRC_Y_POS) | lrc_x, DCSS_SS_DISPL=
AY);
> > > +
> > > +       hsync_start =3D vm->hfront_porch + vm->hback_porch + vm->hsyn=
c_len +
> > > +                     vm->hactive - 1;
...
> > > +       de_ulc_y =3D vm->vsync_len + vm->vfront_porch + vm->vback_por=
ch;
> > > +
> > > +       dcss_ss_write(ss, SYNC_POL | ((u32)de_ulc_y << ULC_Y_POS) | d=
e_ulc_x,
> > > +                     DCSS_SS_DE_ULC);
> > > +
> > > +       de_lrc_x =3D vm->hsync_len + vm->hback_porch + vm->hactive - =
1;
> > > +       de_lrc_y =3D vm->vsync_len + vm->vfront_porch + vm->vback_por=
ch +
> > > +                  vm->vactive - 1;
> > > +
> > > +       dcss_ss_write(ss, (de_lrc_y << LRC_Y_POS) | de_lrc_x, DCSS_SS=
_DE_LRC);
> > > +}
> > > +
> > > +void dcss_ss_enable(struct dcss_ss *ss)
...
laurentiu.palcu%40nxp.com%7C94b752dadd2e413c70d008d740a32979%7C686ea1d3bc2b=
4c6fa92cd99c5c301635%7C0%7C0%7C637048941849367464&amp;sdata=3DeLdnYtMSu7o86=
geKr2wwIlfReLX9EmnGDQXNSuBGebU%3D&amp;reserved=3D0
