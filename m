Return-Path: <linux-kernel-owner@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 4552516A6B0
	for <lists+linux-kernel@lfdr.de>; Mon, 24 Feb 2020 13:59:55 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727854AbgBXM7s (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 24 Feb 2020 07:59:48 -0500
Received: from mx3.molgen.mpg.de ([141.14.17.11]:59145 "EHLO mx1.molgen.mpg.de"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1727329AbgBXM7s (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 24 Feb 2020 07:59:48 -0500
Received: from hypnotoad.molgen.mpg.de (hypnotoad.molgen.mpg.de [141.14.18.16])
        (using TLSv1.2 with cipher ECDHE-RSA-AES128-GCM-SHA256 (128/128 bits))
        (No client certificate requested)
        (Authenticated sender: pmenzel)
        by mx.molgen.mpg.de (Postfix) with ESMTPSA id 3A71320643C4D;
        Mon, 24 Feb 2020 13:59:44 +0100 (CET)
Subject: Re: kernel BUG at arch/x86/kernel/apic/apic.c with Dell server with
 x2APIC enabled and unset X2APIC
To:     Thomas Gleixner <tglx@linutronix.de>,
        Ingo Molnar <mingo@redhat.com>, Borislav Petkov <bp@alien8.de>
Cc:     x86@kernel.org, LKML <linux-kernel@vger.kernel.org>,
        it+linux-x86@molgen.mpg.de
References: <d573ba1c-0dc4-3016-712a-cc23a8a33d42@molgen.mpg.de>
 <87imjzoqhf.fsf@nanos.tec.linutronix.de>
From:   Paul Menzel <pmenzel@molgen.mpg.de>
Message-ID: <98245e23-34c8-899b-93b7-56787817784e@molgen.mpg.de>
Date:   Mon, 24 Feb 2020 13:59:43 +0100
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:68.0) Gecko/20100101
 Thunderbird/68.5.0
MIME-Version: 1.0
In-Reply-To: <87imjzoqhf.fsf@nanos.tec.linutronix.de>
Content-Type: multipart/signed; protocol="application/pkcs7-signature"; micalg=sha-256; boundary="------------ms030602010105010704020509"
Sender: linux-kernel-owner@vger.kernel.org
Precedence: bulk
List-ID: <linux-kernel.vger.kernel.org>
X-Mailing-List: linux-kernel@vger.kernel.org

This is a cryptographically signed message in MIME format.

--------------ms030602010105010704020509
Content-Type: text/plain; charset=utf-8
Content-Language: en-US
Content-Transfer-Encoding: quoted-printable

Dear Thomas,


On 2020-02-21 17:27, Thomas Gleixner wrote:

> Paul Menzel writes:
>>
>> On the Dell PowerEdge T640/04WYPY, BIOS 2.4.8 11/27/2019, Linux 5.4.14=
 (and 4.19.57) with
>> unset `IRQ_REMAP` and `X86_X2APIC` crashes on start-up, when x2APIC is=
 enabled in the
>> firmware.
>=20
>>     [    3.883893] Switched APIC routing to physical flat.
>>     [    3.888904] ------------[ cut here ]------------
>>     [    3.893641] kernel BUG at arch/x86/kernel/apic/apic.c:1616!
>=20
> So the APIC is not registered.
>>
>> `noapic` and `acpi=3Doff` separately did not work, but `noapic acpi=3D=
off` hit the other
>> panic.
>=20
> I have no idea what you are talking about.
>=20
> Which command line options are set to reproduce the above?

Sorry, please ignore my debugging attempts. It fails without any of
these.

> Also please test the latest stable kernels not some random ones.

Please see my reply to Borislav with the logs of Linux 5.6-rc2.


Kind regards,

Paul


--------------ms030602010105010704020509
Content-Type: application/pkcs7-signature; name="smime.p7s"
Content-Transfer-Encoding: base64
Content-Disposition: attachment; filename="smime.p7s"
Content-Description: S/MIME Cryptographic Signature


--------------ms030602010105010704020509--
