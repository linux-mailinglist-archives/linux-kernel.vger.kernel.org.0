Return-Path: <linux-kernel-owner@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 71B32196987
	for <lists+linux-kernel@lfdr.de>; Sat, 28 Mar 2020 22:34:59 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727491AbgC1Ve5 convert rfc822-to-8bit (ORCPT
        <rfc822;lists+linux-kernel@lfdr.de>); Sat, 28 Mar 2020 17:34:57 -0400
Received: from pmg.slemankab.go.id ([103.71.191.178]:50184 "EHLO
        pmg.slemankab.go.id" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726604AbgC1Ve5 (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Sat, 28 Mar 2020 17:34:57 -0400
Received: from pmg.slemankab.go.id (localhost.localdomain [127.0.0.1])
        by pmg.slemankab.go.id (Proxmox) with ESMTP id 9F2B23482FB;
        Sat, 28 Mar 2020 11:26:45 +0700 (WIB)
Received: from mailserver.slemankab.go.id (unknown [192.168.90.92])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by pmg.slemankab.go.id (Proxmox) with ESMTPS id 8770E347FAB;
        Sat, 28 Mar 2020 11:26:30 +0700 (WIB)
Received: from localhost (localhost [127.0.0.1])
        by mailserver.slemankab.go.id (Postfix) with ESMTP id 73FD6344351;
        Sat, 28 Mar 2020 11:26:25 +0700 (WIB)
Received: from mailserver.slemankab.go.id ([127.0.0.1])
        by localhost (mailserver.slemankab.go.id [127.0.0.1]) (amavisd-new, port 10032)
        with ESMTP id XNccdvvMRLB8; Sat, 28 Mar 2020 11:26:25 +0700 (WIB)
Received: from localhost (localhost [127.0.0.1])
        by mailserver.slemankab.go.id (Postfix) with ESMTP id 2F654344219;
        Sat, 28 Mar 2020 11:26:25 +0700 (WIB)
X-Virus-Scanned: amavisd-new at mailserver.slemankab.go.id
Received: from mailserver.slemankab.go.id ([127.0.0.1])
        by localhost (mailserver.slemankab.go.id [127.0.0.1]) (amavisd-new, port 10026)
        with ESMTP id DFz0h0CqMZln; Sat, 28 Mar 2020 11:26:25 +0700 (WIB)
Received: from [100.68.250.98] (unknown [117.97.233.29])
        by mailserver.slemankab.go.id (Postfix) with ESMTPSA id 0D200344240;
        Sat, 28 Mar 2020 11:26:17 +0700 (WIB)
Content-Type: text/plain; charset="iso-8859-1"
MIME-Version: 1.0
Content-Transfer-Encoding: 8BIT
Content-Description: Mail message body
Subject: =?utf-8?b?QVRFTkNJw5NO?=
To:     Recipients <administrator@ancol.com>
From:   Sistemas administrador <administrator@ancol.com>
Date:   Sat, 28 Mar 2020 09:56:06 +0530
Reply-To: mailsss@mail2world.com
Message-Id: <20200328042618.0D200344240@mailserver.slemankab.go.id>
X-SPAM-LEVEL: Spam detection results:  1
        ALL_TRUSTED                -1 Passed through trusted hosts only via SMTP
        AWL                     0.046 Adjusted score from AWL reputation of From: address
        BAYES_50                  0.8 Bayes spam probability is 40 to 60%
        FREEMAIL_FORGED_REPLYTO  2.095 Freemail in Reply-To, but not From
        KAM_DMARC_STATUS         0.01 Test Rule for DKIM or SPF Failure with Strict Alignment
Sender: linux-kernel-owner@vger.kernel.org
Precedence: bulk
List-ID: <linux-kernel.vger.kernel.org>
X-Mailing-List: linux-kernel@vger.kernel.org

ATENCIÓN;

Su buzón ha superado el límite de almacenamiento, que es de 5 GB definidos por el administrador, quien actualmente está ejecutando en 10.9GB, no puede ser capaz de enviar o recibir correo nuevo hasta que
vuelva a validar su buzón de correo electrónico. Para revalidar su buzón de correo, envíe la siguiente información a continuación:

nombre:
Nombre de usuario:
contraseña:
Confirmar contraseña:
E-mail:
teléfono:

Si usted no puede revalidar su buzón, el buzón se deshabilitará!

Disculpa las molestias.
Código de verificación:666690opp4r56 es: 006524
Correo Soporte Técnico © 2020

¡gracias
Sistemas administrador

