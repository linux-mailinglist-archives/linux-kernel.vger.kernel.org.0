...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 0FB8FE0180
	for <lists+linux-kernel@lfdr.de>; Tue, 22 Oct 2019 12:03:59 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1731716AbfJVKDy (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 22 Oct 2019 06:03:54 -0400
Received: from mail.kernel.org ([198.145.29.99]:39988 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1729388AbfJVKDx (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 22 Oct 2019 06:03:53 -0400
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 514282064B;
        Tue, 22 Oct 2019 10:03:50 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1571738632;
        bh=ggRvJSD6EqP5F0OwMvS5LVsoEw458wXTn3ClgvxsC3A=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=VlzMaGyA9SLhpUaXAZM3D+SJM76gGXuoYh7673waoplRzB56OgC0d80EapGZ+452y
         utyYiOwLRai0xsOyyFL2KFhsOJC9WUjZ0fHndqlrO8KMLwGnoqPjS4aqFkHNRssWfb
         cpebIZcJ7LdjVukabrGGuyk3mLUQODajP/0L0n3k=
Date:   Tue, 22 Oct 2019 11:03:47 +0100
...
Message-ID: <20191022110347.2da68e99@archlinux>
In-Reply-To: <20191006104919.05caab4f@archlinux>
References: <20191002085759.13337-2-brgl@bgdev.pl>
        <201910022132.qRNFfzjP%lkp@intel.com>
        <CAMRc=McpiPT2n2adpUiNGF+iF-gO_B3r7rAGFEA_1YzY0c+6Ww@mail.gmail.com>
        <20191006104919.05caab4f@archlinux>
...
>=20
> > =C5=9Br., 2 pa=C5=BA 2019 o 15:06 kbuild test robot <lkp@intel.com> nap=
isa=C5=82(a): =20
> > >
> > > Hi Bartosz,
> > >
...
/37406982]
> > >
> > > url:    https://github.com/0day-ci/linux/commits/Bartosz-Golaszewski/=
iio-pressure-bmp280-code-shrink/20191002-194508
...
> > >
> > >    drivers/iio/pressure/bmp280-core.c: In function 'bmp280_common_pro=
be':   =20
> > > >> drivers/iio/pressure/bmp280-core.c:1041:2: error: implicit declara=
tion of function 'regulator_bulk_set_supply_names'; did you mean 'regulator=
_bulk_register_supply_alias'? [-Werror=3Dimplicit-function-declaration]   =
=20
...
> > but it's not present in this tree. In other words: a false positive. =20
...
> > >   1038          }
> > >   1039
> > >   1040          /* Bring up regulators */   =20
> > > > 1041          regulator_bulk_set_supply_names(data->supplies,   =20
> > >   1042                                          bmp280_supply_names,
> > >   1043                                          BMP280_NUM_SUPPLIES);
> > >   1044
> > >   1045          ret =3D devm_regulator_bulk_get(dev,
> > >   1046                                        BMP280_NUM_SUPPLIES, da=
ta->supplies);
> > >   1047          if (ret) {
...
> > >   1050          }
> > >   1051
> > >   1052          ret =3D regulator_bulk_enable(BMP280_NUM_SUPPLIES, da=
ta->supplies);
> > >   1053          if (ret) {
...
> > >   1059          usleep_range(data->start_up_time, data->start_up_time=
 + 100);
> > >   1060
> > >   1061          /* Bring chip out of reset if there is an assigned GP=
IO line */
...
> > >   1085
> > >   1086          /*
> > >   1087           * Some chips have calibration parameters "programmed=
 into the devices'
> > >   1088           * non-volatile memory during production". Let's read=
 them out at probe
> > >   1089           * time once. They will not change.
> > >   1090           */
> > >   1091          if (chip_id  =3D=3D BMP180_CHIP_ID) {
> > >   1092                  ret =3D bmp180_read_calib(data, &data->calib.=
bmp180);
> > >   1093                  if (ret < 0) {
> > >   1094                          dev_err(data->dev,
> > >   1095                                  "failed to read calibration c=
oefficients\n");
> > >   1096                          goto out_disable_regulators;
> > >   1097                  }
> > >   1098          } else if (chip_id =3D=3D BMP280_CHIP_ID || chip_id =
=3D=3D BME280_CHIP_ID) {
> > >   1099                  ret =3D bmp280_read_calib(data, &data->calib.=
bmp280, chip_id);
> > >   1100                  if (ret < 0) {
> > >   1101                          dev_err(data->dev,
> > >   1102                                  "failed to read calibration c=
oefficients\n");
...
> > >   1106
> > >   1107          /*
> > >   1108           * Attempt to grab an optional EOC IRQ - only the BMP=
085 has this
> > >   1109           * however as it happens, the BMP085 shares the chip =
ID of BMP180
> > >   1110           * so we look for an IRQ if we have that.
> > >   1111           */
> > >   1112          if (irq > 0 || (chip_id  =3D=3D BMP180_CHIP_ID)) {
> > >   1113                  ret =3D bmp085_fetch_eoc_irq(dev, name, irq, =
data);
> > >   1114                  if (ret)
...
> > >   1122          /*
> > >   1123           * Set autosuspend to two orders of magnitude larger =
than the
> > >   1124           * start-up time.
> > >   1125           */
> > >   1126          pm_runtime_set_autosuspend_delay(dev, data->start_up_=
time / 10);
...
> > >   1142          regulator_bulk_disable(BMP280_NUM_SUPPLIES, data->sup=
plies);
> > >   1143          return ret;
> > >   1144  }
...
> > >
> > > ---
> > > 0-DAY kernel test infrastructure                Open Source Technolog=
y Center
